...
| **R18** | T1·A1-F11 (GPT NEW) | MED (static, GPT-only) | `/admin:seed --pointer` trusts an alternate pointer's `instanceDir` (only WarpOS-canonical refused) → corrupted/stale pointer writes warm-start files into arbitrary non-canonical dirs | validate the pointer's instanceDir against an allowed set; EXECUTION |
