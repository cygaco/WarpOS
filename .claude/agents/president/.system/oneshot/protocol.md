...
4. After all builders in a phase complete — run the PARALLEL GAUNTLET:
   a.0 **Integration phase (S1.3) — multi-builder features only, BEFORE the gauntlet.** For any feature built by >1 builder (e.g. `frontend-builder` + `backend-builder`), govern the FE↔BE seam first: write the integration manifest to `runtime/integration/<feature>/manifest.json` (shared files + reconciliation, FE-consumed↔BE-produced type contracts, env/data seam contracts, boundary smoke test, FE/BE merge order/conflict policy — producer defines the shape, consumer adapts) and run the reject-not-lint gate `node scripts/checks/integration-seam-gate.js runtime/integration/<feature>/manifest.json --emit-on-conflict`. exit 0 = proceed; exit 1 = blocking (fix-agent, max 3, re-run); exit 2 = HALT. With no α/β in the room, an unresolved conflict is parked via `scripts/arbitration/emit.js` (owner `gamma_integration`) — the **run-end arbitration ship gate (step 11)** then blocks the run as NOT ship-ready until resolved. Single-builder features skip this. Detail: `.claude/agents/president/gamma.md` → "Integration phase" + `runtime/notes/wave2-s1.3-gamma-integration-phase.md`.
