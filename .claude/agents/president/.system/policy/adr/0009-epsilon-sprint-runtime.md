...
3. `epsilon-runtime.test.js` (42 assertions) + `epsilon-wiring.test.js` (6) exercise every route class, both invariants (incl. a poisoned-registry negative), the override gate, and the script↔ε coverage equivalence — the gated path is covered independent of a live sprint.
4. The runtime writes a completion record reflecting the spawn outcome the caller passes (`ok:false` on a reap); the same `gauntlet-verify` "absence = death" gate + the gamma worktree-diff liveness check remain the ground truth. Real per-agent spawning is staged behind `--epsilon-dispatch` and is OUT of scope for this ADR's record-shaping core (the integration write is proven only against the gitignored worktree runtime, never the canonical ledger, in tests).
