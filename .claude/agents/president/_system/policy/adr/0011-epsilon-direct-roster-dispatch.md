# ADR 0011 — ε summons the in-process roster directly (retiring the ED-041 α-only doctrine)
...
- Extends ADR-0009 (ε sprint-runtime — "ε-the-agent dispatches the in-process roster," the framing this ADR restores) and ADR-0008 (derive-from-registry).
- Capability evidence: 3 agreeing probes + the live Epsilon conductor self-check, 2026-06-18 (E-DISPATCH-PERFECT-001 W5; `DISPATCH-ERRORS.md` F5; memory `feedback_ed041_agent_tool_per_spec`).
