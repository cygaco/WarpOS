...
**Honest residual — a raced post-`fstat` hard link (α ruling under the operator threat model + β discriminator).** The `fstat`→`writeSync` pair is not atomic (there is no POSIX "append-only-if-`nlink===1`" primitive). An attacker who hard-links the fd's inode in the window AFTER the `fstat` and BEFORE the `writeSync` can make the write reach the (now-external) inode. This is **attacker-only** (archive-dir write + a WON microsecond race — no non-malicious mistake hard-links the index), **same-uid**, and **zero-new-capability**: the attacker who can plant that hard link can already write the target file directly. It appends only our index TELEMETRY (not user data). Per the standing mistake-vs-attacker discriminator this is **out-of-model** (adversarial-helm, above the operator's dropped boundary) and is **dispositioned, not mechanised** (α ruling under the operator's "proceed until finished"; β discriminator: zero-new-capability is dispositive). A permanent close is available if the threat model ever expands (write each index entry as an `O_EXCL` unique-named per-entry file — an unpredictable name cannot be pre-hard-linked or raced) but is not warranted under the current model. Tracked at ED-212. The "never writes outside root" guarantee therefore holds for **non-malicious operation + pre-existing hard links**; this raced-link residual is the sole documented exception.
