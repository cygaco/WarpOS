...
| [0034](0034-gate-a-fresh-scaffold-all-ways-sandbox-isolation.md) | GATE-A `fresh_scaffold_all_ways`: real 3-leg installs (`/portfolio:new`, manual `/warp:setup`, shipped `install.ps1`), sandbox-isolated (4 leak vectors + no-delta mechanized proof + pre-run guard); Leg 3 has NO silent node-downgrade (R2); R5 retirement coverage-map (`fresh_install_fixture`/`customized_install_fixture` RETIRED, `update_fixture_from_previous` STAYS); R4 parity-diff reuses test-install-matrix.js's reviewed allowlist. Surfaces (does not fix) two pre-existing, unrelated defects (`_warpos/MANIFEST.json` classifier rot; `install.ps1` bracket-path wildcard bug) — flagged as FOUNDATION-UPDATE-REQUESTs — SP-20260721-001 D-4 INC-2 | 2026-07-21 | proposed | - |
