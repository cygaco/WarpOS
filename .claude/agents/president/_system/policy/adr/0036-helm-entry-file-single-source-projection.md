...
- **GEMINI.md removal-trigger (pinned):** GEMINI.md is a sunset-tombstone kept ONLY while the sunset `gemini` CLI is still live-wired (security-reviewer still routes to it until a later pickup reroutes it to antigravity). When the last gemini CLI consumer is rerouted off the CLI, DROP GEMINI.md and remove its entry from the enforcer's must-exist set. Until then the tombstone is honest-and-relevant (an entering agent can still hit the gemini route this window).
