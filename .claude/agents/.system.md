...
> **Note for WarpOS users:** This document contains the full multi-agent orchestration rules. Some examples reference the source project this framework was first extracted from — those are illustrative. Your project will have different components, features, and file structures. The rules and framework are universal; the examples show how they were applied in practice.
...
- **Gamma** (Alex γ) handles adhoc builds — single features during development. See `.claude/agents/00-alex/gamma.md`.
...
- **Delta** (Alex δ) handles oneshot builds — full skeleton runs with state machine, cycles, points. See `.claude/agents/00-alex/delta.md`.
