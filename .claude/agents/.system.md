...
Builders get: PRD, STORIES.md, FLOW_SPEC.md, COPY.md, INPUTS.md (if exists), DATA-CONTRACTS.md, and the latest HYGIENE.md. Builders do NOT get: golden fixtures, reviewer rubric, step expectations, or any file under `_requirements/00-canonical/fixtures/`.
...
The Orchestrator MUST enforce this by excluding `_requirements/00-canonical/fixtures/` from the file list and context payload when constructing builder prompts.
