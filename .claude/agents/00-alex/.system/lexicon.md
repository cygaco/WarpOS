...
**Adherence rate** — The fraction of suggested skills that the agent actually invoked: `invoked ∩ suggested ÷ suggested`, computed over a rolling window (default 7 days) from the `skill-suggested-vs-invoked` event type. Adherence is filtered to `invocation_path: "ranker"` so manual slashes and unrelated agent-tool calls do not skew the rate. Drives `/check:patterns` analysis and ranker tuning.
