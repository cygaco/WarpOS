...
node scripts/enforcement/betaevents-dedup-lint.js      # SP-20260723-003 / ED-267a (REPORT-ONLY; --enforce → exit 1 on a DEDUP finding): the betaEvents verdict-row integrity lint, /scan:full-scoped ONLY (β: never CI/fresh-clone — betaEvents is a GITIGNORED advisory ledger, absent on a clean checkout → SKIP). Enforces the β-writes-at-delivery pattern (post-SP-002 recipient-drop). (1) DEDUP no msg_id on >1 verdict row (decision ∈ DECIDE|DIRECTIVE|ESCALATE) — keyed on MSG_ID so reconfirms + the "plan->design" vs "plan->design-correction" pair (β rider #1) are NOT false-RED'd, unlike a naive sprint+boundary count. (2) msg_id-RESOLUTION (advisory, β rider #2 fail-open): each msg_id should resolve in the message log (paths.eventsFile); if the log resolves a LOW fraction (<80% — e.g. a truncated-preview log that isn't the real SendMessage log) it SKIPS-with-note, never a wall of false "unresolved". Shares dedup-util.js (P-034). Test: scripts/enforcement/betaevents-dedup-lint.test.js 11/11 (real dup→RED + boundary-distinct/reconfirm→GREEN + low-rate→SKIP).
