...
- `--keep-teams` — skip the team-teardown phase (Phase 8).
...
Run `/beta:mine`. Writes recommendations to the β staging file (`.claude/agents/00-alex/.system/beta/judgement-model-recommendations.md`). Does NOT modify the judgement model directly — Phase 3 reviews the recs. Can run as a background subagent while Phase 1 finishes.
...
### Phase 8 — Tear down teams (`--keep-teams` to skip)
