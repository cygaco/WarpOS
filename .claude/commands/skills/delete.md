...
5. **Maps**: `grep -r "namespace:name" .claude/maps/`
6. **Traces**: `grep "namespace:name" .claude/memory/traces.jsonl` — was this skill used in reasoning chains?
...
The skills map (`.claude/maps/skills.jsonl`) is now stale — staleness detection will flag it automatically. No manual map update needed.
