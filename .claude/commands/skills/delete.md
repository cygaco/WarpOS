...
First, check `.claude/project/maps/skills.jsonl` — find the entry matching this skill and read its `referenced_by` array. This gives you structured inbound references (who calls this skill). If the JSONL map doesn't exist, fall back to `.claude/project/maps/skills.jsonl`.
