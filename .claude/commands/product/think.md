...
# /product:think — Sit With the Project
...
Append a trace to `paths.tracesFile` as `RT-NNN` with `source: "product:think"`. Required: `id`, `ts`, `framework_selected`, `framework_rationale`, `problem` (the meta-question), `root_cause`, `fix` (may be "no action, surface to user"), `quality_score: null`, `open_question` (matches Phase 4 output).
...
- `/reasoning:run` — for a *specific* problem. `/product:think` is broader, meta-direction.
- `/discover:systems`, `/check:all` — surface *state*. `/product:think` surfaces *direction*.
- `/sleep:deep` — REM-phase speculative reasoning. `/product:think` is awake, deliberate, single-pass.
- `/product:bootstrap` — companion entry-point: bootstrap a product brief from a guided discussion (run *before* pondering on a fresh project).
