...
3. **Discover URLs** — From the product URL, do **one-level** internal-link discovery, and parse `sitemap.xml` + nav when present. Prioritize across **source classes** (WG-11):
   - `product` — `/pricing`, `/features`, `/about`, `/changelog`, `/customers`.
   - `landing` — conversion pages, usually richer in JTBD framing + persona/benefit language: `/lp/`, `/solutions/`, `/industries/`, `/use-cases/`, vertical pages.
   - `devdocs` — the real technical surface + integration ecosystem that marketing hides: `developers.`/`docs.` subdomains, `/api`, `/developers`, and any linked Postman/OpenAPI/Swagger reference.
   Hard cap at 8 internal URLs per run, same-host only (devdocs subdomains of the same registrable domain count as same-host) — no recursion. Emit `urls_discovered` (TR-3).
3b. **Collect product imagery** (WG-11, `appimg`) — for a **visual** product the UI imagery IS the product surface. Collect: page `<img>` + `og:image` assets on feature/demo pages, gallery/demo imagery, and **app-store / Play-store listing screenshots** (via the store pages). Read them with a **vision-capable model** into a screen-inventory + UX/design-observations section, and corroborate the feature list against the real UI. Cap images within the per-run fetch budget; skip silently if none found.
...
6. **Extract** — Five LLM passes against the aggregated cached text, in order: JTBDs → feature list → voice-of-customer → gaps → opportunities. Each pass emits `extraction_completed` (TR-6) with `result_count`. Source classes sharpen specific passes (WG-11): **landing** pages strengthen JTBDs + persona/benefit framing; **devdocs** sharpen the scored feature list + the integration ecosystem (often a core moat marketing pages understate); **appimg** (read by a vision model) adds a screen-inventory + UX/design-observations section and corroborates the feature list against the real UI.
...
8. **Emit** — Write `<slug>.clone.md` and `<slug>.clone.html` always; `<slug>.clone.docx` if pandoc is on PATH. Section headings: `## 01 — Product Identity` through `## 07 — Source Attribution Log` (D-2: align with `/portfolio:bootstrap` family conventions so downstream `--from-clone` reads cleanly). The Source Attribution Log tags each record with its `source_class` (`product | review | landing | devdocs | appimg`) so the brief shows coverage across classes — and flags any class that yielded zero sources.
