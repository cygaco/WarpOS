...
**Confirmation is not coverage** — the verification discipline for any control you build, and the highest-recurrence class in this system (three consecutive sprints, S-VLADW1-02/03/04, each closing a class and re-opening it one layer out). Watching a control fire tells you what it CATCHES. It tells you nothing about what it SILENTLY SKIPS, and the fix-quality score of a control verified only by a positive demonstration is capped at 2 — a working instance, not a closed class. Five clauses, each with its own instance:

1. **Enumerate the near-misses before claiming the class.** After observing a catch, the next step is a battery of inputs that *resemble* the protected shape and fail the exact predicate — variant dashes, colons, indentation, spacing, casing, alternate scripts. S-VLADW1-03's claim-bind was verified by watching it catch a real unannounced edit, and its predicate still skipped en-dash, hyphen, colon and indented lead-ins. A sample proves a class OPEN, never CLOSED.
2. **Skip is fail-open; refuse is fail-closed.** Any derivation over authored text must REFUSE what it cannot classify, never continue past it — a near-miss is a violation until proven benign. A skip silently narrows the population and the gate then reports success over the narrowed set. (**ED-358**, open — the mechanical half is a committed near-miss mutation battery that goes RED when the predicate narrows.)
3. **Match the claim's grain to the mechanism's grain.** A coverage claim stated one granularity coarser than the mechanism actually has is its own recurring defect (three instances in a single sprint): "the two scripts the fold covers" asserted a script-level closure over a letter-level sample. State the unit actually enumerated — letters, prefix shapes, paragraph keys — never "scripts" / "all" / "the class", and never a COUNT, which is a property of the day it was read. Sharpened in the β judgment model as P-114/P-115/P-116.
4. **Binding a claim byte-for-byte makes it un-editable, not true.** A byte-bind is an integrity control, not a truth control: if the bound text is false, the bind now ENFORCES the falsehood and an honest correction turns the gate red. Corollary for the author: treat the claim text and its canonical copy as ONE atomic edit owned by ONE bundle, so correcting a bound claim never needs a second commit to make the gate green.
5. **A comment stating an invariant is not an enforcer of it.** S-VLADW1-04's own code comment said two comparison sites "must share one transform"; the transform was applied to one site and the missing fold was implemented BESIDE the other, and nothing detected the divergence — which is what failed the sprint. If the invariant matters, the divergence must be self-detecting (a shared call site, a test that fails on the fork, a lint), not narrated.

**Text matchers cannot tell a violation from a description of one** — the companion trap when clause 2's enforcer is a text scan. Any prose written ABOUT a banned pattern trips the ban: one bundle refused the very document it was editing by quoting the adjacency it was documenting; another turned the tree RED in two files by naming the banned primitive in prose. Both were caught pre-commit, both by accident. Document the trap **in the scanner's own header** so the next author writing about the ban rediscovers it from the code rather than from a red gate, and prefer a structural anchor over a bare literal where one exists.
