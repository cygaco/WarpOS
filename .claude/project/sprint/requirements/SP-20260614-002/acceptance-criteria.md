...
- AC-R1c: Given preconditions are unmet, when `/admin:preview` runs, then it **fails CLEAR with the exact missing step** (`{no Next app / app scaffold absent, npm install failed, dev server not ready/port}`), naming the remediation, and exits non-zero (no silent hang, no open against a dead server, no orphaned dev-server child). AND if the resolved target is the **WarpOS canonical tree** — detected by the `resolved === WARPOS_ROOT` path-belt OR the shared, env-immune resolver `scripts/warpos/repo-role.js#isCanonicalDir` (canonical signals: `_warpos/MANIFEST.json` / `.warpos-canonical` / `warpos.source==="self"` / `warpos.repoRole∈{canonical,framework}` / `project.slug==="warpos"` / `version.json#name`; a consumer's own `warpos:` install-record block with `source != "self"` is NOT a signal — ED-009 single-source, supersedes the original bare-`warpos:`-presence form per β DECIDE 2026-06-15) — it refuses as an **asserted precondition** (`refuseIfTargetIsWarpOS`) before any scaffold/boot.
