...
4. **GEMINI.md** (create) — framework-root-doc shim; ADD `rel === "GEMINI.md"` to build.js framework-root-doc allowlist; embed the marked block; provider-delta = THIN sunset-tombstone: the individual-tier `gemini` CLI is SUNSET; all Gemini routes through Antigravity `agy` → see ANTIGRAVITY.md. RELEVANT NOW because gemini is still live-wired (security-reviewer still routes to the gemini CLI until a later sprint reroutes it). REMOVAL-TRIGGER (documented in the file + ADR-0036): drop GEMINI.md and its enforcer must-exist entry once the last gemini CLI consumer is rerouted off the CLI.
