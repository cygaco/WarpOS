...
- AC-1.1: Given default production configuration, when Vlad obtains model access, then it selects the
  ...
  verified_by: engine/test/model-seam.test.js::selects-subscription-adapter-by-default
- AC-1.2: Given the API-key fallback adapter is selected in a controlled test, when it is exercised
  ...
  verified_by: engine/test/model-seam.test.js::fallback-adapter-contract-parity
- AC-1.3: Given the seam module, when the product tree is scanned, then **no auth-mode conditional or
  ...
  verified_by: engine/test/seam-boundary.test.js::no-auth-mode-literal-outside-seam
- AC-1.4: Given the seam, when it is queried, then it exposes `describeAuth()` returning the live
  ...
  without editing it.
  verified_by: engine/test/seam-boundary.test.js::describeAuth-is-single-source
...
- AC-2.1: Given the stdio server starts, when MCP tools are listed, then the list contains **exactly**
  ...
  verified_by: engine/test/mcp-surface.test.js::lists-exactly-the-four-core-tools
- AC-2.2: Given a caller invokes `send_message`, `approve_job`, `get_roadmap`, or any undeclared tool,
  ...
  verified_by: engine/test/mcp-surface.test.js::refuses-out-of-wave-tools-without-side-effect
- AC-2.3: Given `run_job` receives an unknown or non-read-only job kind, when validation runs, then
  ...
  verified_by: engine/test/mcp-surface.test.js::refuses-unregistered-job-kind
...
- AC-3.5: Given the job state machine, when its transition table is inspected, then it enumerates
  ...
  verified_by: engine/test/lifecycle.test.js::transition-table-enumerates-all-four-states
- AC-3.6 **(the no-exit guard)**: Given every job kind registered in Wave-1, when each is driven through
  ...
  verified_by: engine/test/lifecycle.test.js::no-wave1-job-enters-an-unexitable-state
- AC-3.7: Given a job in any non-terminal state, when `cancel_job` is called, then the job can always be
  ...
  verified_by: engine/test/cancel.test.js::cancel-escapes-any-nonterminal-state
...
- AC-3.1: Given a registered read-only fixture job, when `run_job` succeeds, then it returns a job id
  ...
  verified_by: engine/test/lifecycle.test.js::records-full-transition-path
- AC-3.2: Given a running cancellable fixture job, when `cancel_job` is called, then Vlad records
  ...
  verified_by: engine/test/cancel.test.js::cancelled-only-after-acknowledgement
- AC-3.3: Given a cancellation request that cannot yet stop a worker, when `get_status` is called,
  ...
  verified_by: engine/test/cancel.test.js::pending-not-false-cancelled
- AC-3.4: Given an already-terminal or previously-cancelled job, when `cancel_job` is repeated, then
  ...
  verified_by: engine/test/cancel.test.js::cancel-is-idempotent
...
- AC-4.1: Given a job is killed mid-execution, when the process is restarted, then the target
  ...
  verified_by: engine/test/journal.test.js::kill-midjob-leaves-repo-clean
- AC-4.2 **(write-confinement, quantified over EVERY registered job kind — β supplement `a91c46e2`)**:
  ...
  verified_by: engine/test/journal.test.js::writes-confined-per-registered-kind
...
- AC-5.1: Given an interrupted job in the journal, when `get_status` is called in a new session, then
  ...
  verified_by: engine/test/status.test.js::names-interrupted-job-with-options
- AC-5.2: Given a job id, when `get_status` is called during and after execution, then it returns
  ...
  verified_by: engine/test/status.test.js::never-infers-success-from-completion
...
- AC-6.1: Given a terminal job, when its receipt is returned and journalled, then both representations
  ...
  verified_by: engine/test/receipt.test.js::envelope-shape-is-version-plus-three-slots
- AC-6.2: Given a fixture supplies arbitrary values in each J4 slot, when Vlad records and returns the
  ...
  verified_by: engine/test/receipt.test.js::interior-is-opaque-and-never-branched-on
...
- AC-7.1: Given any child-process launch site, when the custody enforcer runs, then it requires the
  ...
  verified_by: engine/test/custody-static.test.js::spawn-sites-use-allowlist-env
- AC-7.2 **(P3 — one decoy per secret class, per Amendment 1)**: Given the ambient environment is
  ...
  verified_by: engine/test/custody-runtime.test.js::child-cannot-see-decoy-any-class

## S-8 — Fail-closed credential-custody enforcer (governed by ADR-0041, ACCEPTED)

> ### SHIPPED-TREE BOUNDARY (D-3 binding 3) — what P1–P4 actually walk
>
> **The shipped artifact is the ENGINE PACKAGE — `engine/` plus the agent face and installer — NOT the
> repo root.** The seven ED-340 enforcers walk that boundary, and the claim lint covers what ships.
>
> This is the binding that makes P1's scan surface **honest rather than theatrical**. The `vlad` repo
> root carries ~2280 files of a dormant Next.js/Supabase web scaffold that no Wave-1 code touches. A
> repo-root walk would report a large green surface while proving nothing about the thing that actually
> handles the secret — and P1's whole claim is bounded by the surface it enumerates. A wide scan of the
> wrong tree is weaker than a narrow scan of the right one, and reads stronger, which is the dangerous
> combination.
>
> Two consequences to build to: **(a)** the surface manifest is defined over the engine package, so a
> new product-written output path inside `engine/` must join it or P1's coverage silently shrinks; and
> **(b)** `engine/` has its **own `package.json`** and a **near-zero-dependency policy** — every added
> dependency needs a recorded A1 justification in the same commit, because A1 (dependency surface) is
> the largest residual and any package in the tree can read `process.env` and reach the network. Next,
> React and Supabase never enter `engine/`'s dependency graph.
>
> Test paths below are anchored under `engine/test/` (runner: built-in `node --test`, zero added
> dev-dependencies), which is also why ADR-0041's P3 identity `test/credential-custody-decoy.test.js`
> resolves correctly relative to the engine package root.
...
- AC-8.1: Given the declared scanned product surface contains a held-secret value or a seam-declared
  ...
  verified_by: engine/test/custody-static.test.js::fails-on-secret-in-scanned-surface
- AC-8.2: Given a raw `spawn` / `exec` / `fork` (or equivalent) bypass of the audited wrapper is
  ...
  verified_by: engine/test/custody-static.test.js::fails-on-raw-spawn-bypass
- AC-8.3: Given a dynamic `require`/`import` with a computed specifier in the product tree, when the
  ...
  verified_by: engine/test/custody-static.test.js::bans-computed-specifier-imports
- AC-8.4: **(Mutation twin — non-negotiable.)** Given the wrapper's scrub or env allowlist is removed
  ...
  verified_by: engine/test/custody-runtime.test.js::negative-fixture-goes-red-when-scrub-removed
- AC-8.5: Given the enforcer's own execution errors, times out, or emits malformed output, when the
  ...
  verified_by: engine/test/custody-static.test.js::fails-closed-on-runner-error
- AC-8.6: Given the packaged product runs on a user machine, when the server or job runner starts,
  ...
  verified_by: engine/test/custody-runtime.test.js::selfcheck-runs-on-user-machine
- AC-8.7 **(P4 — `scripts/checks/no-secret-on-outbound.js`)**: Given the product's outbound-request call
  ...
  verified_by: engine/test/no-secret-on-outbound.test.js::sdk-auth-call-is-sole-carrier
- AC-8.12 **(P4's mutant — the second mutant ED-340 closes on)**: Given a **planted non-auth outbound
  ...
  verified_by: engine/test/no-secret-on-outbound.test.js::planted-outbound-carrier-goes-red
- AC-8.8: Given a parse error in any scanned file, when the enforcer runs, then it goes RED rather
  than skipping the file.
  verified_by: engine/test/custody-static.test.js::parse-error-is-red-not-skip
- AC-8.9 **(claim lint — ADR-0041 labeling rule)**: Given any custody claim string in shipped copy
  ...
  verified_by: engine/test/claim-lint.test.js::every-custody-claim-maps-to-a-p-clause
- AC-8.10 **(A1–A4 presence obligation)**: Given the shipped custody statement, when the claim lint
  ...
  verified_by: engine/test/claim-lint.test.js::a1-a4-ceilings-present-verbatim
- AC-8.11 **(A5 firing point, presence-checked)**: Given the product's release gate, when it runs,
  ...
  verified_by: engine/test/custody-runtime.test.js::release-gate-asserts-shiptime-wiring
...
- AC-9.1: Given a recognized successful provider result, when normalized, then the outcome is success.
  verified_by: engine/test/quota.test.js::recognized-success
- AC-9.2: Given a recognized quota-exhaustion signal, when normalized, then the outcome is
  ...
  verified_by: engine/test/quota.test.js::recognized-quota-exhaustion
- AC-9.3: Given an **unrecognized** signal, when normalized, then the outcome is `could-not-run`, the
  ...
  verified_by: engine/test/quota.test.js::unrecognized-is-could-not-run-with-raw-signal
- AC-9.4: Given the signal `Server is temporarily limiting requests (not your usage limit)`, when
  ...
  verified_by: engine/test/quota.test.js::capacity-limit-is-not-quota
- AC-9.5: Given the three buckets, when the seam flips subscription↔API-key, then bucket
  ...
  verified_by: engine/test/quota.test.js::normalization-lives-in-seam
...
- AC-10.1: Given a permission-level value outside the ported vocabulary, when Vlad starts, then
  ...
  verified_by: engine/test/permission.test.js::invalid-level-refuses-at-startup
...
- AC-11.1: Given the planning/read-only permission level, when `run_job` is requested, then Vlad
  ...
  verified_by: engine/test/permission.test.js::readonly-level-refuses-run-job
- AC-11.2: Given that same level, when `get_status`, `get_readiness` or `cancel_job` is requested,
  ...
  verified_by: engine/test/permission.test.js::readonly-level-keeps-query-and-cancel
...
- AC-12.1: Given a release or packaging check, when `branding-identity-enforcer` scans user-facing
  ...
  verified_by: engine/test/branding.test.js::fails-on-claude-code-or-missing-attribution
- AC-12.2: Given the shipped MCP/server-facing text, when the branding check passes, then the product
  ...
  verified_by: engine/test/branding.test.js::identifies-as-vlad-powered-by-claude
...
- AC-13.1: Given a clean local checkout with no MCP host, agent face, installer or credentials, when
  ...
  over stdin/stdout.
  verified_by: engine/test/driver.test.js::exercises-four-tools-without-host
- AC-13.2: Given the driver fixture job runs, when the repository fixture is compared before and
  ...
  records.
  verified_by: engine/test/driver.test.js::target-repo-unchanged-after-run
