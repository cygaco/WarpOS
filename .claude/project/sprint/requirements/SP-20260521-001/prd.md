...
- `R-1` — **Paths registry expansion.** Add `portfolioRegistry (removed paths key)` (→ `~/.warpos/portfolio.json`), `paths.portfolioHome` (→ `~/.warpos`), `paths.briefsRoot` (→ `_docs/briefs/`), `paths.clonesRoot` (→ `_docs/clones/`) to `framework/paths.registry.json`. Propagate to canonical `.claude/paths.json`. Schema bump to `v5` if needed (currently v4).
