...
- **Recommended:** Option (a) dispatch-from-subprocess via simple file-drop polling. Orchestrator drops `consult-request-<phase>.json` in `betaInbox/`, polls `betaInbox/consult-verdict-<phase>.json` with timeout (default 5min). Runtime (smart-context or a new poller-hook) picks up requests, dispatches Beta via Agent, writes verdict. No long-running listener; file-drop is cron-able + tracked via existing event sinks. Includes timeout halt with `halt_reason: beta_consult_timeout` and clear remediation.
- **Expanded:** recommended + persistent runtime poller that watches `betaInbox/` continuously (PostToolUse hook + background) + Beta-consult batching (multiple consult-requests dispatch in parallel) + verdict caching (skip Beta if previous consult on same phase + same hashed input verdicts DECIDE within last N minutes) + telemetry dashboard (per-sprint Beta consult latency histogram).
