...
|---|---|---|
| C1 | **Event compactor** `scripts/events/compact.js` — folds old events into a summary + keeps a bounded raw TAIL; compacted raw MOVES to archive.js (never deleted). Idempotent, single-writer/non-racy (SP-001 lock pattern), fail-closed (any doubt → archive-not-delete). | GC-1: round-trip test proves EVERY raw event is archived+indexed (0 lost); restore-drill recovers raw; fail-closed on a mid-compaction crash (no partial loss). |
