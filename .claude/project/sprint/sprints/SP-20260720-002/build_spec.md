...
`scripts/dispatch/falsifiers/*.falsifier.test.js` (Phase-3 shape: skip-RED until unit lands → full-valid context so the gate is REACHED → attack ONE variable → REACHABILITY proof [exact reason / spy-called / positive control], never bare outcome===false). `scripts/checks/check-lib-single-source.js` (β rider 4 enforcer — greps zero inline re-impls of any CHECK_NAMES entry outside check-lib/checks/, asserts all 3 consumers require() check-lib; exit 1 on re-impl/drift; wired into /scan:full). **β R2:** it MUST ALSO assert the pinned bundle manifest's `promotion.from_src_digest` === the live check-lib source digest — the AUTHORITATIVE consumer (the controller) runs the pinned SNAPSHOT, not require(check-lib), so without the lineage check a fork could ship in the snapshot unverified-as-same-source. (buildBundle in Unit BUNDLE stamps `promotion.from_src_digest` = the source digest it snapshotted.) CORE-2 flip: `.claude/kernel/fixtures/trust-boundary/trusted-layer-sole-integrator.json` report_only:true→false, then `conformance-matrix.js --flip-gate` (require authorized:true), wire --enforce. Honest-promise statement + close ED-215 (note ED-236 H5). Owner LAST = backend.
...
**β R1 — CHECK-SET PROVENANCE (BINDING, must land before the CONTROLLER reconcile is gate-passing):** `mintRunManifest` derives `expected_checks`/`required_checks` FROM the pinned bundle's FROZEN `CHECK_NAMES`/`REQUIRED_CHECKS` (read from the pinned bundle manifest), NEVER from `input.expected_checks`. `runPinnedSuite` executes the pinned bundle's FULL CHECK_NAMES, never a caller-supplied subset. `input.expected_checks?` may ONLY additively constrain — assert `input ⊆ bundle.CHECK_NAMES` and it may require MORE, but it can NEVER shrink `required_checks` below the bundle's `REQUIRED_CHECKS`. Without this, `expected_checks=[]` (or a shrunk-required set) VACUOUSLY passes reconcile — nothing runs, the empty/shrunk-set bypass on the sole route into main. Falsifier: `caller-cannot-shrink-check-set.falsifier.test.js` (S4) — `[]`/shrunk over a tree that FAILS a required check → still REFUSED, asserting the pinned required check FIRED.
