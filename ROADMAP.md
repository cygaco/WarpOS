...
>
> **Priority ordering (Director-of-Product ranked 2026-06-07, golden-path-first):** **(1)** E-DISPATCH-INTEGRITY-001 **F-1/F-2** (release-blocking, already scoped — land NOW, do NOT freeze behind the analysis gate) → **(2)** ⭐⭐ E-MC-READINESS-ANALYSIS-001 (frozen-target gate; run hardening-sim + prose-vs-reality first) → **(3)** E-GOLDEN-FLOW-001 (the clean-machine golden path) → **(4)** E-CONTENT-DELIVERY-001 (close ship slip-through) → **(5)** E-MC-READINESS-EXECUTION-001 (gated; burns down frozen findings) → **(6)** E-TEST-SUITE-001 (parallel enforcement, non-blocking). **Bundling:** E-GOLDEN-FLOW-001 → analysis track-1 (hardening); E-CONTENT-DELIVERY-001 → track-4 (release-pipeline); E-DISPATCH-INTEGRITY F-4/F-5 → track-5 (file-org — same work as ED-026, route once); E-TEST-SUITE-001 stays standalone. Cleanup (DoP): E-SPINUP-STEPS-001 is Completed (move out of Active); E-TRACKER-001 close T4→Completed; E-MANAGER-LAYER-001 restate as ~30% residual. Post-launch / parked: E-STABLE-CHANNEL / E-BOUNDARY / E-MULTIPRODUCT / E-SKILL-CATALOG.
...
- **Priority:** ⭐⭐ TOP (operator-directed 2026-06-07 — the gate on shipping masterconsole externally). **State:** Active. **Completion:** 0%.
...
- **Priority:** High (in-flight; finishes the "#3 big rock"; ranked do-next #4). **State:** Active. **Completion:** ~60% (verified 2026-06-06). DONE & ENFORCED: ship-coverage is **exhaustive** (`warpos-ship-coverage.js` runs green — 1304 paths, 0 hard_gaps / 0 info_gaps / 0 boundary_violations, info_gaps a hard failure, reviewed `KNOWN_NOT_SHIPPED` allowlist) and **`seeded_from` integrity** is zero-tolerance (0 dangling, `KNOWN_DANGLING_SET` empty). OPEN: `_warpos/templates/` + `_warpos/BASELINE/` build + `framework/templates` migration (DoD #1, verified absent); provenance-seeding of `_requirements/`/`_docs/` (DoD #3, `populate-source.js` absent); install-matrix update-parity (DoD #4, partial — scenario 2 + post-update checks exist, full REQUIRED_DIR assertion unconfirmed).
...
- **[partial — cheap-slice shipped 2026-05-29] Artifact-first, contract-tested release gate** — install ONLY the sealed capsule into a disposable out-of-tree product repo and run an EXECUTABLE consumer contract (`setup → scan:install → a real sprint → dispatch-telemetry → update → guard behavior under BOTH repo roles`) before shipping. Stronger than a static manifest diff: it consumes the exact artifact downstream receives. **✅ Cheapest leading indicator shipped (the Director's keystone, do-first):** `scripts/warpos/test-fresh-install-smoke.js` — spins up a fresh empty repo, runs the consumer install (warp-setup + the framework-manifest asset the install.ps1 path lays down), asserts `scan:install` certifies it (exit 0); fail-closed on a stale manifest; ~3s, per-commit (discovered by `scripts/linters/run.js`). **First run caught a real bug:** every fresh install baked `manifest.warpos.version = "0.1.0"` (≠ `version.json`) — a version-quorum mismatch — fixed in `scaffold-core.js#resolveWarposVersion` (reads the installing source's `version.json`). **Still open (the full gate):** sealed-capsule isolation (canonical inaccessible) + the executable consumer contract (real sprint → dispatch telemetry → update, under BOTH repo roles) — scoped to the 0.18.0 stable-promotion gate below.
...
*Optimization frame (operator, 2026-06-01): rank for WarpOS's **dual role** — (1) **engine for masterconsole** (imminent dependent launch; engine reliability that protects it is core-loop per the current-stage carve-out), and (2) **dev tool for the downstream products** (dreamteam/companycam/almanac/forerunner/dogly, on 0.8–0.10). Each item is labeled **[ENGINE-for-MC]**, **[DEV-TOOL-downstream]**, or **[BOTH]**. **Stage: `pre-mvp`** (declared; resolver `scripts/warpos/lifecycle-stage.js`) — max-leanness; the only "hardening" that counts as core-loop is what keeps a pinned capsule from breaking MC's launch + what fixes the paths downstream products actually hit.*

> ✅ **RESOLVED (operator, 2026-05-30): freeze-vs-pull is a *launch-time* call, decided by WarpOS's stability at the time — it may freeze OR pull.** Because **pull is a live possibility**, the **consumer-contract gate (#1) is the keystone *either way*** — it (a) makes a pull-posture safe, (b) certifies a freeze snapshot, and (c) produces the very stability signal that informs the launch-time freeze/pull call. The order below stands as written; #1 is no longer conditional.

**🔑 Keystone (now):** the **executable consumer-contract gate** **[BOTH]** — the open half of the artifact-first release gate (the cheap-slice fresh-install smoke shipped `ff04cd9`, which caught a real `manifest.warpos.version = "0.1.0"` bug on first run). Prove the *exact sealed capsule MC pins* survives a real consumer lifecycle (`setup → scan:install → real sprint → dispatch telemetry → update`, **both repo roles, cold + warm/update paths**) before external users hit it. Single item that certifies MC's pin AND the path every downstream `/warp:update` hits; structural fix for the "contractless productization" root behind ~every WARPOS.md gap (`lifecycle-aware-judgment` + carve-out; `evidence-over-invention`).

**Ranked do-next:**
1. **Executable consumer-contract gate** (sealed-capsule isolation + both-role, cold+warm lifecycle) — **[BOTH]** — the keystone; certifies the artifact MC pins and the downstream update path. *(`lifecycle-aware-judgment` + engine carve-out; pre-mvp core-loop reliability.)*
2. **Shared repo-role resolver** (ED-009) — **[BOTH]** — one canonical-vs-consumer source every guard reads instead of re-deriving role "from path vibes"; structurally unblocks #1/#3 and kills a recurring false-green class. *(`focus`/`lean` — smallest unlock with the most leverage.)*
3. **Typed success semantics** (green = action occurred AND telemetry exists) — **[BOTH]** — kills fail-open false-green at the contract level (BC-16); without it the gate in #1 can itself lie. *(`evidence-over-invention`.)*
...
- **[shipped 2026-05-29] Compare-and-contrast / diff WarpOS ↔ a product repo (e.g. masterconsole).** `/warp:diff <slug|--product PATH>` (engine `scripts/warpos/diff.js#computeDiff`) diffs canonical framework against an installed product: installed version + staleness (`product_behind`/`ahead`/`same`), framework-file drift sub-classified **`product_stale`** (unchanged-since-install, canonical moved on — a clean `/warp:update` fixes) vs **`product_modified`** (locally edited — an update would conflict) vs `missing_in_product`, coverage gaps (missing/extra framework files), and skills/agents/hooks Δ by `kind`. Builds on the portfolio registry + `framework-installed.json` asset hashes via `content-hash`. **Read-only / canonical-side** per [[feedback_warpos_only_no_cross_project]] — reads the product, never edits it; tested against synthetic fixtures only (no real sibling touched). 24 unit tests (every classification + compareVersions + resolution errors), auto-discovered by `scripts/linters/run.js`. *(Feeds 0.17.0 — ships with its exhaustive suite.)*
...
- **[open] Enforce a feedback-block on roadmap items until operator or WarpOS-user input.** A roadmap-governance mechanism to mark certain items as **blocked pending feedback** and *enforce* it: a blocked item cannot be picked up / planned / sprinted until the required feedback is recorded. Two feedback sources — **(a) the operator** (me), and **(b) a downstream WarpOS user/consumer** (e.g. a product maintainer who must weigh in before canonical builds something on their behalf). Shape: a roadmap marker like `[blocked: awaiting-feedback(operator|warpos-user) — <question>]` + a **named enforcer** (per CLAUDE.md § Policy & Enforcement Hygiene — this item *is* an enforcer, so its mechanism must be named, not aspirational): a `/sprint:plan` / sprint-pickup guard (hook or skill-step) that refuses to start a sprint whose roadmap source-item is feedback-blocked until the block clears. Open design: how feedback is **recorded to clear the block** — a `/roadmap:unblock <id> --feedback "…"` skill, a COMMS.md-style ledger (overlaps the proposed `collab:` suite's partner-comms channel), or a `_planning/` feedback entry; how a downstream WarpOS user's feedback reaches canonical (via `/warp:flag` → WARPOS.md, or a new feedback channel); and how blocked items surface (`/roadmap:next`, `/scan:full`, sprint pickup). Builds on the existing `/scan:roadmap-trace` + sprint-plan gates. *(Surfaced 2026-05-29, operator.)*
...
- **[open] New build mode: "raw"/vanilla Claude (no team, no process restrictions) — and name it distinctly.** Extend the build-mode system (`solo` · `adhoc` · `oneshot`) with a mode that is **just default Claude Code** — NO α/β/γ team, NO gauntlet / β-gate / sprint-pipeline ceremony, minimal framework process — for when the operator wants raw model behavior without the framework's overhead. **Distinct from `solo`:** solo is still Alpha-the-WarpOS-identity with the reasoning/hooks/memory layer (just no *team*); this mode strips the *process* too. **Naming (operator cares — "call this 'mode' something else"):** give it its own term, not a generic "[X] mode" — candidates: `raw` · `vanilla` · `bare` · `plain` · `freehand` · `passthrough` (operator to pick). **Key connection:** this mode IS the operational **"WarpOS-off"** condition — it's the **A/B baseline for the `bench:` suite** (WarpOS-on vs -off, above) and the cheapest way to answer "what does the framework actually buy?" (the adhoc-assessment item). Pairs with the **mode-switcher** (auto-pick the cheapest mode per task — this becomes the floor) and applies to **sprints** too (a sprint run in this mode skips the gauntlet/ceremony). Open design: HOW to turn the framework off cleanly — a settings kill-switch (hooks off, smart-context off, no team) vs a stripped `.claude` clone; what STAYS (memory? on-demand skills?) vs goes (team, gauntlet, β-gate, sprint pipeline); and whether it's a session mode (`/mode:<name>`) and/or a per-sprint flag. *(Surfaced 2026-05-30, operator.)*
...
**[open] Canonical docs still describe another product (Jobzooka) — scrub it out + make canon describe WarpOS itself. (DISCOVERED-2026-06-02)** The "maintainer's canonical scrub" named in the status note above is now concretely blocking work. `/scan:framework-purity --full` reports **69 `client_slug` ("Jobzooka") hits + 5 `abs_path` (maintainer home-dir paths, mostly in old `framework/releases/*` upgrade-notes) + 2 `promote_relic`** across the canonical checkout (3205 files scanned). This session it **hard-blocked a commit**: the `framework-purity-guard` (gate #2 above) correctly refused the item-1 `.system.md` change because `.claude/agents/.system.md:3` carries an illustrative-examples note referencing `Jobzooka`, so the RI-004 dispatch-method prose rewire had to be **dropped** to land the functional fix. Root cause: canonical's `_requirements/00-canonical/*` (and agent/system docs) are WarpOS-as-product specs *borrowed from a real downstream product* — they describe/exemplify **Jobzooka**, not WarpOS itself. Two coupled fixes: **(a)** scrub every `Jobzooka` / maintainer-abs-path / promote-relic leak from canonical (generalize the `.system.md` examples; lift WarpOS-as-product specs into a private repo per the status note), and **(b)** rewrite the canonical canon to describe **WarpOS's own** identity/product-model, not a stand-in project. Until both land, `/scan:framework-purity --full` stays RED and any commit touching a Jobzooka-bearing file is blocked. **Why it persisted silently:** the commit-time `framework-purity-guard` runs `--diff` (changed files only), so it catches *new* leaks but never forces remediation of the 69 *pre-existing* ones — consider a release-gate `--full` purity check (fail-closed) so the backlog can't be ignored forever. *(Origin: RI-004 build, 2026-06-02 — the `.system.md` commit-block; pairs with the status-note scrub + Pickup Queue § "Sprint 10+ candidates".)*
...
**[open] Scan-suite reconciliation — remaining items (post SP-20260531-004).** *(2026-05-31)* The self-inventory enforcer (`/scan:scan-coverage`) shipped; these surfaced in the ponder/reasoning pass and were deferred to keep that sprint lean: (1) **`scripts/check/` → `scripts/checks/` consolidation** — 4 stragglers (`install`, `node-procs`, `privacy`, `roadmap-trace`) live in the minority dir; a multi-ref rename (skills + both manifests + any hooks) — do as its own sprint with a full OLD-path grep-sweep (rename-hygiene bug class). (2) **`warpos-*` umbrella** — 16 distribution-integrity scans (40% of the suite; 4 share `preflight.js`) → group into a navigable domain (a `/scan:warpos-integrity` that delegates, like `/scan:full`) + per-scan domain tags in the scan registry so overlap is legible. (3) **Namespace normalization** — the `warpos-` prefix is applied inconsistently (`install`/`version-coherence`/`framework-*` are distribution-integrity without it). (4) **dead-but-shipped detector** — extend `/scan:references` to flag files that ship via manifest but have NO live consumer (the roadmap-doc class: `.claude/project/reference/warpos-roadmap.md` + `warpos-launch-roadmap.md` ship to every product but nothing reads them; `references` counts manifest refs as "live", so it misses them — this also closes the operator's 2026-05-31 "roadmap artifacts everywhere" cleanup). (5) **stale/dead-worktree scan** — nothing flags git worktrees behind canonical or on merged/dead branches (the exact state this session ran from). Priority: low-medium — hygiene, not correctness; each is independently shippable.
