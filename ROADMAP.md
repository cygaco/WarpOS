...
- **[open] Wire the regression-seed enforcer into the SPRINT pipeline (0.17.0 completion gap).** 0.17.0 shipped `scripts/testsuite/enforce.js` wired into `release-gates.js` (so `/warp:release` gates on the regression seed) — but it is NOT called from `scripts/sprint/full.js` or `execute.js`. So `_docs/sprint/TESTSUITE.md`'s convention ("every WarpOS sprint keeps the regression seed green") has a named enforcer at RELEASE but **none at sprint-close** — the aspirational-vs-enforced gap (BC-15) recurring *inside* the system built to close it. Fix: call `enforce.js` (canonical-only via `isCanonical()`) at `sprint:full` Phase-4/close + `sprint:execute`'s record step; on a NEW regression, halt/flag per the autonomy preset. Completes 0.17.0's "per-sprint enforced" promise. *(Surfaced 2026-05-29 — γ qa verified the enforcer is release-only, not sprint-lifecycle; confirmed by grep of scripts/sprint/.)*
