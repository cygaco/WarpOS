...
> ⚠️ **CONDITIONAL on an open operator decision — "MC beta: pull engine updates, or freeze on a verified snapshot?"** (escalated below). The order assumes the riskier **pull-updates** posture. If MC **freezes on a snapshot**, #1 collapses to a one-time snapshot certification and the standing keystone shifts to #4 (0.16.0) + #6 (0.17.0 foundation). Director recommends **freeze for the beta**.
