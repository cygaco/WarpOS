...
### Prioritized order — Director of Product (2026-05-29)

*Generated by `/roadmap:prioritize` (Director of Product, 9 principles). **Stage: `pre-mvp`** (declared; resolver `scripts/warpos/lifecycle-stage.js`) — max-leanness, prove the core loop — reconciled with WarpOS being the **engine under Master Console, which ships to external users soon on a pinned engine**. So the only "hardening" that counts as core-loop work is what keeps a pinned capsule from breaking MC's launch; genuine scale/breadth hardening stays deferred.*
...
**🔑 Keystone (now):** the **executable consumer-contract gate** — the open half of the artifact-first release gate (the cheap-slice fresh-install smoke shipped `ff04cd9`). Prove the *exact sealed capsule MC pins* survives a real consumer lifecycle (`setup → scan:install → real sprint → dispatch telemetry → update`, **both repo roles, cold + warm/update paths**) before external users hit it.

**Ranked do-next:**
1. **Executable consumer-contract gate** (sealed capsule + both-role, cold+warm lifecycle) — the keystone; certifies the artifact MC pins. *[MC launch readiness / pre-mvp core-loop reliability]*
2. **Shared repo-role resolver** — one canonical-vs-consumer source every guard reads; unblocks #1/#3 and kills a recurring false-green class (ED-009). *[simplicity/reliability]*
3. **Typed success semantics** (green = action occurred AND telemetry exists) — kills fail-open false-green at the contract level (BC-16). *[trustable "green" on the pinned artifact]*
4. **0.16.0 Content-Delivery Integrity** (ship-coverage + seed-with-provenance + update parity) — closes "downstream always missing"; in-flight, finishes the #3 big rock. *[consumer install path / MC clean-pin]*
5. **Lifecycle/retirement registry** — retired capabilities governed by a registry guards read, not re-litigated per session. *[simplicity]*
6. **0.17.0 Test-Suite System foundation + runnable regression seed** — scoped to harness + seed + sprint-close enforcer ONLY; defer the `_planning`/diff/hook-overhaul batch. *[engine reliability MC depends on]*

**Defer (pre-mvp says not now):** `bench:` suite · adhoc-vs-solo assessment + `/mode:auto` (paired) · `collab:` suite · turbo legibility/cadence · `_planning`/diff/hook-overhaul (keep inside 0.17.0, behind the foundation) · roadmap-governance items still "needs operator input to scope" (feedback-block, feedback pipeline, suite-centering) · maintainer canonical scrub (operator-scoped) · finish (don't expand) the in-flight DoP/roadmap-skills.

**Confidence 0.78.** Order-changer = the freeze-vs-pull decision (above). **Escalations (operator decides):** (a) **freeze-vs-pull** — Director recommends freeze-on-snapshot for the beta + build #1 as the gate that certifies the *next* pin; (b) pulling 0.18.0's stable-promotion slice forward as the keystone is a sequencing call worth a β sanity-check before planning.
