...
1. **E-DISPATCH-SHAPE-001 W2 core** — ⏩ **REPORT-ONLY SLICE LANDED 2026-06-16 via SP-20260616-001** (the shared `shapeDoor()` gate + all 4 dispatch entry points consult the resolver report-only; `WARPOS_SHAPE_DOOR=report|enforce` per-wrapper ramp + `WARPOS_DISABLE_SHAPE_DOOR` kill-switch; `dispatch-skill` PINNED report-only [ED-057]; 33 planted assertions both modes + 108 regression green; cross-provider gauntlet GPT-5.5 FAIL→fix→PASS; dispatch-guide §16.9). **Remaining (operator-gated, HIGH-BLAST — NOT auto):** the per-wrapper ENFORCE FLIP (one advisory-noise-zero sprint each) + dispatch-contract ENFORCE flip + route-guard repoint + envelope ADR-0012. On the Master Console critical path (SG-C).
