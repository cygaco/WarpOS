...
1. **E-DISPATCH-SHAPE-001 W2 core** — ⏩ **REPORT-ONLY SLICE LANDED 2026-06-16 via SP-20260616-001** (the shared `shapeDoor()` gate + all 4 dispatch entry points consult the resolver report-only; `WARPOS_SHAPE_DOOR=report|enforce` per-wrapper ramp + `WARPOS_DISABLE_SHAPE_DOOR` kill-switch; `dispatch-skill` PINNED report-only [ED-057]; 33 planted assertions both modes + 108 regression green; cross-provider gauntlet GPT-5.5 FAIL→fix→PASS; dispatch-guide §16.9). ⏩ **W2 ENFORCE WINDOW LANDED 2026-06-16 (this session, autonomous — the operator's "fully finish" mandate satisfied the gating; 9 commits on `main`, latest `7760ab84`, blast-sensitive changes cross-provider GPT-5.5 gauntlet-verified):** per-wrapper shape-door ENFORCE flip on all 3 agent wrappers (`958d0960`, FAIL→fix→PASS on 4 findings incl. fail-open-not-preserved) + **dispatch-contract gate ENFORCE flip (`7760ab84`, ADR-0013, β DECIDE 0.87 FLIP-not-supersede — the non-redundant gate for api-when-CLI/in-process-hard/cwd/forbidden_shapes)** + route-guard N5 + quote/path/escape evasion-hardening (`6839d6ef`/`e9dd6bad`/`449bd6c4`, 2 gauntlet rounds) + envelope ADR-0012 bury (`e7c84735`). ALSO the related-deferred: ED-058 model-chain enforcer + directors→xhigh + ED-057 subprocess-skill (`21848be5`); concurrency-lock cwd cap-defeat + sprint-state atomic writes (`8339e0b5`). **W2 NOW FULLY DONE. REMAINING for 100%: W3 per-failure-class review-lane policy ONLY.** Epic ~92%. On the Master Console critical path (SG-C).
