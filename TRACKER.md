...
**EXECUTION session 1 — the additive dispatch-shape FOUNDATION (PLAN §17.2 steps 1–3) SHIPPED 2026-06-08** (Active, ~25%). Built + green-tested + GPT-5.5 cross-provider-reviewed (FAIL→fixed across 2 rounds), all ADDITIVE: P5 fixture harness (`scripts/checks/lib/fixture-harness.js`); N-3 auth-resolver (`scripts/dispatch/auth-resolver.js`); safety kernel (`scripts/dispatch/safe-spawn.js`); dispatch-contract KEYSTONE §17.1 (`.claude/agents/_org/dispatch-contract.json` + `scripts/dispatch/dispatch-contract.js`); N-1 coverage gate + run-ledger (`scripts/dispatch/coverage-gate.js` + the `run_id`/`shape`/`prompt_digest` stamps on both wrappers). `/scan:full` wired report-only. 64 test assertions, 35 planted-violations. **NEXT ACTION:** (a) refactor the existing wrappers/`providers.js` to ROUTE THROUGH safe-spawn + auth-resolver (currently new-code-only — the lib-only-fix-doesn't-protect-bypassing-callers rule); (b) S-2 dispatch-guide consolidation + the §17.2 destructive tranche (de-dot S-3, renames S-7) behind a full isolated dry-run (§14 + §17.6 preconditions); (c) flip the report-only enforcers to blocking (§4 step 8). Backed by o3 deep-research (`runtime/research/dispatch-subprocess-safety/`) + GPT-5.5 (×3 incl. the 2 security-review rounds, `runtime/agent-system-plan/review/`); ED-033 logged.
