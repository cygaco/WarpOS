...
- **W4 RESTRUCTURE** (gut canonical `_requirements/` to product-agnostic + relocate the example → `_warpos/EXAMPLES/`) — LANDED @`c89a73a7` (2026-06-22, Option-A operator-authorized). A **deeper-W4 domain-vocab residual** (~19 canonical + 38 `_warpos/BASELINE/` example-client files + EXAMPLES not yet ship-walked) is honestly tracked as its own judgment-heavy follow-on sprint @`9bbc9119` — W4's "name-clean" claim is honest, NOT a false "fully product-free".
