...
Last Updated: 2026-06-07 (session/2026-06-06: shipped mode-init-≠-authorization fix; WI-50 installer fix → released 0.15.1; tracker initializer `/trackers:init` + ship-templates + enforcer-shippability gate (BC-30); cross-provider dispatch diagnosis → NEW ⭐ TOP roadmap epic **E-DISPATCH-INTEGRITY-001**. Prior: E-TRACKER-001 + E-SPINUP-STEPS-001 COMPLETED 2026-06-06; validator 20/20)
...
**⭐ E-DISPATCH-INTEGRITY-001 (Agent Dispatch & Orchestration Integrity) is the TOP active roadmap epic** (created 2026-06-07 from a cross-provider dispatch diagnosis; operator-directed). **Next action = F-1:** make a backing `ok:true` completion record (the `gauntlet-verify` predicate) the PRECONDITION for any sprint phase to count as "covered" — touch `scripts/sprint/full.js:95-116`, `.claude/commands/sprint/full.md:93`, `scripts/checks/sprint-manager-consult.js`, `scripts/checks/sprint-hook-coverage.js`. This kills the deepest defect (RC-2: `/sprint:full` emits coverage telemetry with zero real dispatch — "sprint theater" wired into the default) and turns the sound-but-uncalled `gauntlet-verify` into an actual gate. Full root-cause analysis + F-1..F-5 + the cross-provider evidence: `trackers/epics/E-DISPATCH-INTEGRITY-001-agent-dispatch-integrity.md` + `.claude/runtime/diagnostics/diag-{claude,gpt2}.json`. (Decide at F-1 design: default `/sprint:full` to `--epsilon-dispatch` vs. reject the non-dispatch coverage path at the enforcer — prefer the more fail-closed enforcer-side change.)
