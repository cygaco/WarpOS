...
## Definition: Seam
Definition: A boundary between two parts of the system that cannot share state directly and must connect through an explicit contract. Two principal kinds. (1) **Integration seam** — the producer/consumer data boundary where one feature's output feeds another's input: the producer defines the exported shape (a type, function, or field), and the consumer imports and adapts, never inspecting the producer's implementation ("the producer defines the shape; the consumer adapts"). Authoritative: `.claude/agents/_system/guides/integration-seam-contract.md`; owned by the Director of Engineering (the Gamma integration phase, S1.3), not by feature builders. (2) **Process/runtime seam** (the "node seam") — the boundary between execution contexts that cannot pass live coordination: e.g. a node subprocess (`/sprint:full`) cannot reach the in-process harness team's `SendMessage`/`Agent` tool (ED-041), so work dispatched across it ORPHANS unless an explicit bridge carries the result back (the dispatch/return invariant — a conductor that dispatches background work then returns without bridging the result orphans it).
Why it matters: Seams are where independently-built parts compose — and where they silently fail. A producer that widens a shared type without updating the store's `lockedInterfaces` breaks every downstream consumer; a conductor that returns across the node seam without bridging orphans the dispatched work (the sprint-team-orphaned and judgment-pipeline-unwired failure classes). Get the seam contract right and two isolated agents compose without either reading the other's code; get it wrong and the break is invisible until a downstream consumer fails.
Where it applies: FE/BE integration (frontend-builder / backend-builder as consumers/producers; the Director of Engineering's integration phase owns the commons); cross-context dispatch (the `/sprint:full` halt-and-bridge β consults, the ε conductor's dispatch/return, the Master Console orchestrator→renderer EVENT-CONTRACT). Guardians: `integration-seam-gate.js` (the FE/BE seam) and the dispatch invariants (`safe-spawn` / the dispatch-shape resolver) for the node seam.
