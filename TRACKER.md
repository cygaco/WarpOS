...
**⭐ OPERATOR-DIRECTED (2026-06-19, session 2) — THREE workstreams, newly tracked + survive-checked (full prescriptive handoff: `DUMP.md` 2026-06-19 + `REPORT-JULY-18.md` at root):** (1) **W4 RECONCILED + RE-SCOPED** — the real W4 = product-agnostic AGENTS + SYSTEM; verified this session the agent specs are essentially CLEAN (1 incidental file `president/_system/oneshot/ENV-SETUP.md`; the apparent 23 domain-vocab hits were false-positive "resume"-the-verb), and the residual SYSTEM coupling is the hardcoded linters (`lint-prds.js`/`lint-stories.js`/`lint-hl-stories.js` + `hooks/lib/project-config.js`) → parameterize from each product's canon/slug. SEPARATELY the `_requirements/` example product-spec was de-branded jobzooka→AcmeLaunch (DONE, branch `sprint/SP-20260619-001-jobzooka-genericize` @ `99e033c8`, jobzooka=0/branded=0) and RE-SCOPED per operator — gut the canonical core to project-agnostic + keep AcmeLaunch as ONE labeled example under `_warpos/EXAMPLES/` (the prior session's "replace-in-core with AcmeLaunch" was a mis-scope of W4). Surgical gut PENDING (per-file partition; templates carry examples to genericize). (2) **E-TEAMS-MIGRATION-001** (NEW — registered in `ROADMAP.md` § Active epics) — Claude Code v2.1.178 (2026-06-15) removed `TeamCreate`/`TeamDelete` (teams now implicit + session-scoped, spawn via `Agent(name, run_in_background)`, nesting ≤5); fix ~5 skills + ~3 hooks + tests + ~12 docs + subprocess cleanup + orchestration-doctrine + a regression enforcer. Nothing crashes. (3) **W5+W3** — β-CLEARED (DECIDE B 0.89, code-verified); ready to execute `runtime/w5-impl-staged.md` with β's 5 riders + the ADR-0011 filename-collision fix (two ADRs numbered 0011 → renumber roster ADR to 0014), then ADR-0013 repair → W3. The 2026-06-17 block below remains valid for the broader E-DISPATCH-PERFECT epic.
