...
**⭐⭐ OPERATOR-DIRECTED TOP EPIC (2026-06-17): E-DISPATCH-PERFECT-001 — Perfecting Agent Dispatch.** Authored 2026-06-17 via `/epic:plan` ([trackers/epics/E-DISPATCH-PERFECT-001-perfecting-agent-dispatch.md](trackers/epics/E-DISPATCH-PERFECT-001-perfecting-agent-dispatch.md) · plan `_planning/epics/E-DISPATCH-PERFECT-001.md`), registered at the TOP of ROADMAP § Active epics. The 2026-06-17 session was design + re-verification (no code shipped): the Opus-4.8-max model chain was confirmed ALREADY DONE (`model-chain.js` exit 0, role-parity clean, 35 spec pins — no rebuild), and **ED-041 was FALSIFIED** on the current harness (live probe — teammate-ε spawned director-of-product + product-lead + a depth-3 nest cleanly; the only block was the WarpOS `scope-contract-guard` policy gate) → the R1/R7 hierarchy redesign collapsed to "ε summons the roster directly + scopeContract + ADR" (recorded in `DISPATCH-ERRORS.md` F5, marked FALSIFIED). The NEXT session must EXECUTE the ship-now waves: **W0** (model-catalog freshness: `check.js` 30→14d + honest age + test · remove gpt-5.3-codex · frontmatter effort pins β-xhigh/γ-δ-high · ED-058 frontmatter-blind-spot · panel registry-derived rows) · **W1** (3-provider security review — Gemini+GPT+Claude best-of-each; WIRE THE FIRING, not key-only, per β DECIDE 0.88 conditions a–d) · **W2** (GPT product-leadership chain — director-of-product/product-lead/design-lead/director-of-growth + a NEW `{tier:director,provider:openai}→cross_provider_consult_lead` class-derivation rule + `fallback:claude`) · **W5** (ε-summons-roster + ADR retiring ED-041). W3 (codex-doer migration — new `subprocess-codex` build shape) is GATED on E-DISPATCH-SHAPE-001's ADR-0013 enforce-flip repair; W4 (jobzooka genericization) runs parallel. Full prescriptive pickup: `DUMP.md` (2026-06-17). The 2026-06-16 items below remain valid where not superseded by this epic.
