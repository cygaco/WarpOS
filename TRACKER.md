...
**Design + execution entry point (2026-06-08, planning session):** the full analysis/design now lives in `runtime/agent-system-plan/PLAN.md` (§0–§17) with `GO.md` as the start doc. The operator reframed this into the broader **WarpOS dispatch-shape reliability system** (agents + skills = one problem: the best dispatch shape per unit). **S-1 AUDIT is DONE.** On the operator's "go", begin at **PLAN §14 Phase 0**, then the §17.2 build order (safety kernel + dispatch-contract keystone + fixture-harness + file-usage-trace BEFORE any destructive rename/delete). Backed by o3 deep-research (`runtime/research/dispatch-subprocess-safety/`) + two GPT-5.5 consults; ED-033 logged.
