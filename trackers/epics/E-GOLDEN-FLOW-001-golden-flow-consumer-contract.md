...
- [ ] Dispatch-readiness preflight (`provider-smoke.js --per-role`) wired into `/warp:health` + SessionStart + `/agents:test` — PARTIAL (the `--per-role` ping is built but not wired into all three entry points; tracked as ranked do-next #5).
