...
- **Percent completion:** 65% — build of the seven items landed, gauntlet-1 evidence exists and is adjudicated, and fix attempt 1 is in flight with 2 of 5 bundles landed `ok:true`. The number is NOT higher because S2/S3/S5 still fail on the last measured commit, three bundles have not returned, and no gauntlet has yet been run against the fixes.
