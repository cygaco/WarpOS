...
> **Authored:** 2026-06-08 (adhoc mode, α + β consult + 6 parallel Phase-0 investigators).
...
### 8.8 Turbo Mode
Spend ceiling ($ default — value is the §22 operator call), a permission profile (auto / notice / confirm / never) covering commit/push/merge/spend/branch/deps/auth/env/secrets/delete/migrate/deploy, a spend ledger, and a "resolve-don't-defer" integration with β (β must not casually recommend deferring the core work in turbo — folds into the existing β-honesty Tier-4). Respects all hard ceilings; classifier-compatible (no broad `node-e-fs` self-grant).
...
- **S-LC-07 — Turbo hardening.** Spend ledger + ceiling, the auto/notice/confirm/never permission profile, the safe commit/push/merge grant (operator-gated default per §22), β "resolve-don't-defer" integration. Classifier-compatible. **Deps:** S-LC-01. **∥:** S-LC-06. **Gate:** operator sign-off on the push/spend default (§22).
...
2. **Turbo auto-push/merge + spend default** (source §F wants $100 + commit/push/merge-to-main). Conflicts with CLAUDE.md Autonomy ("Push = Ask first") + the classifier broad-scope denial. **Operator decision:** (a) keep push as ask-first even in turbo [safest], (b) turbo grants push-to-current-branch but not main, (c) turbo grants push+merge-to-main within safety rails [source's ask]. Plus the default spend ceiling value ($100 per source, vs the CLAUDE.md ≥$5 floor).
3. **Provider funding/subscription detection method** — (a) self-attestation [recommend default], (b) opt-in billing-API probe, (c) infer-from-dispatch. And the source's own open Q: T3 `Max 5x` vs a lower hobbyist Claude sub.
