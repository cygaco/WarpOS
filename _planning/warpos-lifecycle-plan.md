...
2. **Turbo auto-push/merge + spend default** — operator intent RESOLVED 2026-06-08 ("highest autonomy possible"); **DELIVERY feasibility-corrected the same day** (GPT-5.5 + α proof — see §1A). Turbo auto-grants spend (default $100, operator-raisable; this session $1000) + commit + branch ops. **Push/merge-to-main CANNOT be auto-granted mechanically** — `permissions.allow` already contains `git push *` yet the harness classifier still gates push-to-main on per-action intent. So push stays per-action (a one-typed-line-per-arc confirmation, or operator-run `! git push`). Residual sub-calls for execution: the framework-default spend ceiling value ($100 baseline); a live preflight mapping which scopes the classifier honors.
