...
## 1A. Independent Review & Feasibility Findings (GPT-5.5 cross-provider + α, 2026-06-08)

This plan was reviewed after authoring by **GPT-5.5** (cabinet role, `ok:true`, real cross-provider dispatch) and an independent **α feasibility check** — they CONVERGED. **GPT-5.5 verdict: NEEDS-REWORK** — on OVERCLAIMS in the enforcement layer, NOT the foundation (registry-first + extending the existing dispatch-matrix / team-guard / provider stack is validated by both). β confirmed the rework (DECIDE 0.91). The four findings are folded into the sections below as honesty corrections (none invalidate the epic — they make three over-sold guarantees honest before any code is written; principle 6 / 15):

1. **Turbo auto-push is NOT mechanically deliverable (PROVEN).** `Bash(git push *)` is already in BOTH `.claude/settings.json` and `settings.local.json` `permissions.allow`, yet the harness auto-mode classifier still blocked a push-to-main this session. The classifier sits ABOVE `permissions.allow` and gates push-to-main on per-action intent; WarpOS cannot configure it away. → §8.8 / §22 / S-LC-07 re-specced: turbo auto-grants spend + commit + branch; **push-to-main stays per-action**.
2. **Mode-switch interception is incomplete; the fix is a single transaction writer.** A PreToolUse `/mode:*` guard misses the handoff-text / direct-`mode-set.js` paths, so it can't carry the "mechanical, not memory-dependent" claim alone. → §8.3 re-specced: `mode-set.js` (the single mode writer) owns the lifecycle transaction; the PreToolUse guard is **BACKSTOP-ONLY** (catches bypass), explicitly not co-primary (β).
3. **"Mechanical team teardown" → best-effort.** The harness can't force-kill a live in-process teammate and team state is machine-global. → §8.4 / §25 downgraded to best-effort + reconcile.
4. **Blast radius is the deepest risk (GPT's "most dangerous thing").** These gates fire on every mode switch / dispatch / turn while depending on new registry state — one bad parse could block all work. → §20 reinforced: fail-OPEN on guard/parse exceptions, fail-CLOSED only on explicit validated bad states; report-only ramp; a recovery path independent of the registry.

Evidence: GPT-5.5 review at `runtime/agent-system-plan/lifecycle-review/gpt55-review.out`; the permissions.allow proof is reproducible (`grep 'git push' .claude/settings*.json` → present in both, yet push-to-main was classifier-blocked).
...
### 8.3 Mode Switching
**Primary enforcement = the single lifecycle-transaction writer (feasibility-corrected 2026-06-08, GPT-5.5):** `scripts/mode-set.js` (the ONLY mode writer) performs the lifecycle transaction inline — preflight (detect active team) → teardown-old (slug-scoped, best-effort) → verify → init-new per registry → verify → persist → emit the event sequence. Putting the transaction in the single authoritative writer closes the hole a PreToolUse-only design leaves (handoff-text / direct-`mode-set.js` paths that never go through the slash command). **The PreToolUse `/mode:*` guard is BACKSTOP-ONLY** (β 2026-06-08): it catches rogue tool-calls that try to dispatch/switch around `mode-set.js` — explicitly NOT co-primary (the writer owns the transaction; the guard catches bypass; the two are not redundant, so neither gets deleted in a cleanup sweep). Idempotent (repeat `/mode:sprint` ≠ duplicate team). Fail-closed only on explicit validated bad states; fail-OPEN on guard/parse exceptions (§13).

### 8.4 Persistent Team Lifecycle
`scripts/teams/lifecycle.js`: project-scoped identity (slug filter on `~/.claude/teams/*`), spawn/verify, **best-effort** kill/teardown, duplicate prevention, orphan detection, durable readiness record (`.team-live-<sid>` + a state file), `SessionEnd` hook backstop, resume reconciliation. **Honest ceiling (feasibility-corrected 2026-06-08, GPT-5.5):** teardown is BEST-EFFORT + reconcile, not guaranteed-mechanical — the harness cannot force-kill a live in-process teammate (`TeamDelete` can't kill a running process) and team state is machine-global, so project-scoping is probabilistic (a slug heuristic) unless the harness itself stores project identity. The manager therefore: request shutdown → `TeamDelete` → mark stale/orphaned state → block/reconcile future dispatch; it never claims a guaranteed kill. Answers the prompt's 12 team questions (§ findings 02).
...
### 8.8 Turbo Mode
**Operator decision 2026-06-08 (β-escalated Class C, resolved): "highest autonomy possible."** Turbo auto-grants the MAXIMUM autonomy short of the never-allowed hard ceilings: an operator-set spend ceiling (framework default $100, raisable per session — this session was $1000), commit, push, AND merge-to-main + branch ops, within the safety rails. A permission profile (auto / notice / confirm / never) still governs deps/auth/env/secrets/delete/migrate/deploy, with a spend ledger and a "resolve-don't-defer" β integration (β must not casually recommend deferring the core work in turbo — folds into the existing β-honesty Tier-4). The never-allowed ceilings stay never (sign-up/purchase, secret exposure, unauthorized destructive ops, backup-branch deletion). **Implementation constraint — FEASIBILITY-CORRECTED 2026-06-08 (GPT-5.5 review + α proof):** auto-push-to-main is NOT mechanically deliverable from inside WarpOS. **PROOF:** `Bash(git push *)` is ALREADY in both `.claude/settings.json` and `settings.local.json` `permissions.allow`, yet the harness auto-mode classifier still blocked a push-to-main this session — the classifier sits ABOVE `permissions.allow`, distinguishes a standing grant from per-action intent, and WarpOS cannot configure it away. **Honest ceiling:** turbo auto-grants spend + commit (+ branch ops within the classifier-honored scopes); **push/merge-to-main ALWAYS requires per-action operator intent** (a confirmation, or the operator running `! git push`). So S-LC-07's real deliverable is (1) the spend ledger + ceiling + the auto/notice/confirm/never profile for classifier-honored scopes, and (2) a one-typed-line-per-push-arc flow for push (the cheapest safe form) — NOT an auto-push the classifier rejects. The operator's "highest autonomy" intent stands; the DELIVERY is bounded by the harness classifier, and a live preflight must map exactly which scopes the classifier honors before S-LC-07 flips anything.
...
- **S-LC-07 — Turbo hardening.** Spend ledger + ceiling, the auto/notice/confirm/never permission profile for **classifier-honored scopes** (spend/commit/branch), and a **one-typed-line-per-push-arc** flow for push (push-to-main is NOT auto-grantable — see §8.8 proof), plus the β "resolve-don't-defer" integration. **First task = a live classifier preflight** mapping exactly which scopes the harness auto-mode classifier honors vs gates per-action. **Deps:** S-LC-01. **∥:** S-LC-06. **Gate:** operator intent = highest autonomy (2026-06-08); delivery bounded by the classifier (feasibility-corrected 2026-06-08).
...
| Turbo auto-push is classifier-blocked (PROVEN 2026-06-08) | Push-to-main stays per-action — the harness classifier sits above `permissions.allow` (which already grants `git push *`); turbo auto-grants only classifier-honored scopes; a live preflight maps which scopes are honored |
| Blast radius — gates fire every turn, immature registry in the hot path (GPT's top danger) | Fail-OPEN on guard/parse exceptions, fail-CLOSED only on explicit validated bad states; report-only ramp per mode; recovery path independent of the registry; kill-switch |
...
1. **Epic ID + title** (`E-LIFECYCLE-001`, this title) — naming is a taste/semi-irreversible call. *Recommend approve.* **→ RESOLVED 2026-06-09 (operator): APPROVED.**
2. **Turbo auto-push/merge + spend default** — operator intent RESOLVED 2026-06-08 ("highest autonomy possible"); **DELIVERY feasibility-corrected the same day** (GPT-5.5 + α proof — see §1A). Turbo auto-grants spend (default $100, operator-raisable; this session $1000) + commit + branch ops. **Push/merge-to-main CANNOT be auto-granted mechanically** — `permissions.allow` already contains `git push *` yet the harness classifier still gates push-to-main on per-action intent. So push stays per-action (a one-typed-line-per-arc confirmation, or operator-run `! git push`). Residual sub-calls for execution: the framework-default spend ceiling value ($100 baseline); a live preflight mapping which scopes the classifier honors. **→ RESOLVED 2026-06-09 (operator): APPROVED with a $100 framework-default spend ceiling (raisable per session). Turbo auto-grants spend + commit + branch ops within the never-allowed hard ceilings; push/merge-to-main STAYS per-action (the harness classifier sits above `permissions.allow` — proven). Encoded as DP-gap #39(a) operator-declared-override clause + (b) classifier-above-permissions layering fact in `paths.decisionPolicy`.**
3. **Provider funding/subscription detection method** — **β-DECIDE 2026-06-08 (conf 0.84): (a) self-attestation as the default + (b) an optional opt-in billing-API probe; reject (c) infer-from-dispatch as default** (silently wrong on quota-exhaustion — succeeds once, fails under load). Operator-confirmable. Still open (source's own Q): T3 `Max 5x` vs a lower hobbyist Claude sub. **→ RESOLVED 2026-06-09 (operator): APPROVED self-attestation default + optional opt-in billing-API probe; REJECT infer-from-dispatch. T3 tier-value question DEFERRED to Wave 4 design.**
4. **Backward-incompatible mode behavior** — any mode whose *disallowed work* list would block something currently allowed (e.g. solo retool) needs sign-off before its gate flips to blocking. **→ DEFERRED 2026-06-09 (operator): resolved at the Wave-1 gate-flip, not now.**
5. **`epic:` public command names** — the 10-command suite names (`/epic:plan` … `/epic:repair`). **→ RESOLVED 2026-06-09 (operator): APPROVED all 10 (`/epic:{plan,start,status,fold,split,close,review,acceptance,link,repair}`); build `/epic:plan` + `/epic:fold` first.**
6. **Playbook direction** — reference-procedures (recommended) vs executable-protocols (`/playbook:run`). **→ RESOLVED 2026-06-09 (operator): APPROVED reference-procedures; design `/playbook:run` later, do not build in this epic.**
