# GPT 2nd-pass security re-review — E-LIFECYCLE-001 Wave-1 lane (S-LC-03/04/05, landed june-9, REPORT-ONLY)

You are a SECURITY REVIEWER performing a cross-provider 2nd pass on the Wave-1 mode-lifecycle enforcement surfaces. The original Wave-1 security verdicts were gemini-only (codex was reaped); this GPT pass is the owed cross-family check before the report-only->blocking flip.

SCOPE: the two W1 deliverables below — mode-lifecycle-guard.js (the PreToolUse /mode:* preflight guard) and session-end-team-teardown.js (the SessionEnd best-effort teardown backstop). Both are wired REPORT-ONLY BY DESIGN — do NOT flag report-only as a defect (the flip is operator-gated).

Look for REAL security defects: bypassable gates, trust-without-verify, fail-open where it should fail-closed, command/path injection, missing attestation on kill-switches, non-circular mode validation (a gate must not trust mode.json to decide whether to trust mode.json), project-slug scoping holes (foreign-team teardown).

VERDICT: end with a single line `VERDICT: PASS` or `VERDICT: FAIL`, then a bullet list of findings tagged BLOCKER / MAJOR / MINOR. If PASS, state briefly why the surfaces are sound.


===== FILE: scripts/hooks/mode-lifecycle-guard.js =====
#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────────────────────
...
===== END scripts/hooks/mode-lifecycle-guard.js =====

===== FILE: scripts/hooks/session-end-team-teardown.js =====
#!/usr/bin/env node
...
===== END scripts/hooks/session-end-team-teardown.js =====
