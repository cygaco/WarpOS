...
**Recorded:** ED-408. Candidates named there: the detector reports *could-not-check* when the store is absent rather than reporting findings; the suite records the checkout kind beside each class and refuses cross-kind comparison; the stale-marker warning becomes an error under a strict flag.
...
**Entries: thirteen.** Each names its recording location. This list's completeness is bounded by the derivation rule above and by nothing else; a residual that fails any of the rule's three conditions is absent from it by construction and not by judgment.
