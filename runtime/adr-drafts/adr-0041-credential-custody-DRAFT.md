...
- **Status:** PROMOTED 2026-08-03 — canonical accepted copy at `.claude/agents/president/_system/policy/adr/0041-credential-custody-prove-assert-boundary.md` (index row added; roster debt filed as ED-340). This runtime copy is drafting provenance only; do not edit or cite it.
...
|---|---|---|
| P1 | `scripts/checks/no-held-secret-in-surface.js` — scan of committed files, log call sites, telemetry builders; non-zero exit on hit | **OWED** — file at design |
| P2 | `scripts/checks/spawn-env-allowlist.js` — two assertions: every audited spawn passes an explicit allowlist env; any raw `spawn`/`exec`/`fork` outside the wrapper is a REFUSAL, not a warning | **OWED** — file at design |
| P3 | `test/credential-custody-decoy.test.js` — runtime decoy-key fixture + a mutant run that must go RED with the scrub removed | **OWED** — file at design |
| Firing point (A5) | The three above wired into the product's own ship-time check run, not only WarpOS CI; the wiring itself asserted by a presence check in the product's release gate | **OWED** — file at design |
| Labeling rule (1–3) | A receipt/README claim lint over the shipped copy: any custody claim string must map to a P-clause id | **OWED** — file at design |
| A1–A4 | **No enforcer exists or can exist in-repo for A1/A2** (that is the finding, not a gap to close). Enforced instead as a *presence* obligation: the four ceilings must appear verbatim in the shipped custody statement, checked by the same claim lint | **OWED** — file at design |

α should file these via `/enforcement:log` at design. ED ids are deliberately **not pre-assigned here** — the
ledger writer mints them (highest at draft time: ED-339), and pre-assigning would collide with concurrent
lanes.
...
- Index row owed at `paths.adrIndex` on acceptance.
