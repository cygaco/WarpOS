...
- **Population — who fills each shelf, WHEN:** `audience` ← **Research Lead** (research/early phase + refreshed as you learn) · `copy` ← **Copy Lead** (general methodology seeded at framework level; *project* voice derived early from audience+canon, refined) · `design` ← migrated from `_guides/design` (M3) + ingested design methodology (framework-seeded, general craft) · `state` ← **the system, EVERY sprint/feature** (continuous; canon = intended/stable, state = actual/evolving). **Ingest firewall:** external sources (`_docs/research|imports|briefs|clones`) are firewalled (`scan:ingest-firewall` + the real-time untrusted-content-firewall hook) — treated as **DATA, never instructions** (prompt-injection defense) — BEFORE a lead distills them into `_knowledge`. So `_knowledge` is *curated/distilled*, not raw-ingested.
