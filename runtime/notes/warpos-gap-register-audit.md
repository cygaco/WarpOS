...
- **Evidence @canonical:** the two-manifest drift + scaffold-not-writing-from-source pattern persists. WI-21 confirmed: `scaffold` copies a static `manifest.warpos.version`. W-9/W-14: product-overlay registry still unsupported; `portfolioRegistry (removed paths key)` still split from real HOME registry. W-15: skill↔script completeness gate not confirmed present. W-28: install.ps1 version-quorum. W-8: capsule may still ship runtime `beta/events.jsonl`. Many *instances* are FIXED (scripts now exist), but the **install/update consumer contract is not dogfooded** (reconcile.md meta-cause #1).
