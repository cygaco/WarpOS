# security-reviewer (GPT 2nd-pass, cross-family) — SP-20260611-002 security surfaces (G1 lifecycle gates, G2 turbo-auth, W1-fold)

You are a SECURITY REVIEWER doing the binding cross-provider pass. These surfaces ARE enforcers (BC-16: a wrong fix is an enforcement regression). Review for REAL security defects: bypassable gates, trust-without-verify, fail-open where it should fail-closed, command/path injection, require()-redirection, non-circular mode validation (a gate must not trust mode.json to decide whether to trust mode.json), project-slug/cwd scoping holes, unauthenticated kill-switch suppression, turbo self-lockout / monotonic-or-attested re-apply, spend-anchor integrity, nonfinite-bytes handling, the node-e-fs safety floor (rm/unlink not approvable + tracked-delete floor).

SPECIFIC W1-FOLD FOCUS (these came from a prior GPT FAIL — confirm they're genuinely fixed): (T-324) session-end-team-teardown.js must anchor projectDir to the hook's OWN repo root (NOT event.cwd/env) so a foreign event.cwd cannot redirect require(); (T-325) mode-lifecycle-guard.js team-membership must be exact-project-cwd or member-cwd-UNDER-project, NOT parent-containment.
...
@@ -109,10 +110,14 @@ function matchNodeEFs(toolName, ti) {
...
@@ -187,6 +267,15 @@ function isInSafetyFloor(toolName, ti) {
...
@@ -208,70 +297,91 @@ function readAuth() {
 }
...
+  isInSafetyFloor,
+};
diff --git a/scripts/hooks/mode-lifecycle-guard.js b/scripts/hooks/mode-lifecycle-guard.js
index e458ee0..5553ecb 100644
--- a/scripts/hooks/mode-lifecycle-guard.js
+++ b/scripts/hooks/mode-lifecycle-guard.js
...
@@ -169,8 +205,15 @@ function resolveSessionId(projectDir) {
...
@@ -199,8 +242,10 @@ function findActiveTeamForProject(slug, projectDir, homeDir) {
...
@@ -306,9 +351,20 @@ function run(event, opts) {
...
+  attestModeGuardKillSwitch,
   ...
   resolvePrevMode,
   gitLabels,
diff --git a/scripts/hooks/session-end-team-teardown.js b/scripts/hooks/session-end-team-teardown.js
index 76b5690..8245f8b 100644
--- a/scripts/hooks/session-end-team-teardown.js
+++ b/scripts/hooks/session-end-team-teardown.js
@@ -14,14 +14,38 @@
...
@@ -142,24 +165,13 @@ function teamCarriesConductor(conductor) {
       "teams",
     );
...
@@ -170,6 +182,143 @@ function teamCarriesConductor(conductor) {
   return false;
 }
...
@@ -279,10 +459,41 @@ process.stdin.on("end", () => {
...
+    let inAdhocMode = currentMode === "adhoc" || modeContradictsHarness;
     if (!inAdhocMode) {
       const adhocDir = path.join(
         process.env.HOME || process.env.USERPROFILE || "",
diff --git a/scripts/mode-set.js b/scripts/mode-set.js
index f340e05..efa4cce 100644
--- a/scripts/mode-set.js
+++ b/scripts/mode-set.js
...
+  membersCoverFaces,
   ...
   projectTeams,
diff --git a/scripts/turbo/apply.js b/scripts/turbo/apply.js
index 35017e1..5068f7a 100644
--- a/scripts/turbo/apply.js
+++ b/scripts/turbo/apply.js
...
@@ -181,6 +188,62 @@ function parseSpendCeiling(s) {
   return n;
 }
...
diff --git a/scripts/turbo/spend-ledger.js b/scripts/turbo/spend-ledger.js
index 4a7ab00..8e24ca7 100644
--- a/scripts/turbo/spend-ledger.js
+++ b/scripts/turbo/spend-ledger.js
