# CROSS-FAMILY RE-REVIEW — SP-20260610-005 F-1+F-3 coverage-honesty (debt clearance)

You are the cross-family (gemini) lens on E-DISPATCH-INTEGRITY-001 F-1+F-3, which shipped with claude-only review during your quota outage (2026-06-10). It passed claude execution-access lanes after TWO fix-cycles (both found live-reproduced false-greens: sprint-only correlation greened by a sprint_id-less historic record; unparseable --since/--until silently widening to whole-ledger scan — both now fail-closed).

THE WORK (final landed diff): F-1 = scan:sprint-manager-consult + scan:sprint-hook-coverage require a correlated ok:true completion record for post-2026-06-10 sprints (RECORD_BACKED_CUTOFF legacy exemption; null-window fail-closed). F-3 = gauntlet-verify requires --sprint or a PARSEABLE window; whole-ledger verify refused exit 2; sprint-only mode excludes sprint_id-less records.

ATTACK: remaining false-green/false-red paths the claude lanes missed — cutoff comparison edges (string vs date), window-buffer arithmetic, records with crafted role/time fields, the legacy-exemption boundary, regex/parse divergences between the two F-1 checks (they share logic by copy — drift risk).
OUTPUT: fenced JSON envelope {"verdict":"PASS|FAIL","blockers":[{"file":"...","line":"...","issue":"...","severity":"blocker|major|minor"}],"notes":"<=300 chars"}

--- BEGIN DIFF ---diff --git a/scripts/checks/sprint-hook-coverage.js b/scripts/checks/sprint-hook-coverage.js
index 534bec6..cb5d4d9 100644
...
@@ -65,6 +77,30 @@ function validateIsoDate(val) {
...
@@ -211,10 +256,19 @@ function computeFindings(events, sprintDates = {}, cutoff = WIRE_DATE) {
...
+const cp = require("child_process");
 ...
 const {
   verifyGauntlet,
@@ -836,6 +837,187 @@ test("FIX5 (cwd-regression real): canonicalFile is cwd-independent after chdir +
   }
 });
...
@@ -852,6 +1034,7 @@ if (failures.length) {
