# REVIEW — SP-20260610-003 Lanes C+D (sprint-pipeline truth + research:deep runnability), canonical WarpOS

Integrated diff (2 build units, base dfef6e8) fixing five verified bugs:
- WG-3: /sprint:full skill body bypassed the org roster; engine epsilon-dispatch was opt-in. Fix: default ON when isSprint() (registry-backed mode detection; explicit flags/env win; non-sprint modes byte-identical) + org-era skill rewrite + design_without_roster report-only enforcer.
- WG-7: design scaffold minted PRD R-lists inconsistent with stories/trace R-refs. Fix: both derive from plan contract requirement_areas (dynamic N) + fail-closed checkTraceIntegrity at scaffold time.
- AL-W-006: status.js missing crash_recovery/ralph/reports fields vs current.yaml schema; checkpoint.js returned existing unvalidated. Fix: schema align + normalizeExisting().
- MC-WG-2: /research:deep skill used harness-blocked primitives (bash sleep polling, node -e fs-writes). Fix: NEW standalone scripts/research/deep-run.js (internal async polling, all fs-writes in-script, injectable transport seam, 36-case test with zero live calls) + skill rewritten as thin wrapper (Claude leg + synthesis stay in-skill).
- MC-WG-3: research Phase 0 checked model access not credit. Fix: tiny billable probe per provider (max_tokens<=5) classifying insufficient_quota up front.

ATTACK SPECIFICALLY:
1. The epsilon-dispatch default flip: can it activate OUTSIDE sprint mode (e.g. isSprint() misread, env fallthrough)? Can an explicit --no flag be overridden by the default? Are the planted tests real (would they fail if the guard were wrong)?
2. checkTraceIntegrity: can it false-green (R-id regex misses formats, partial file reads) or false-positive on legit legacy markers?
3. design_without_roster enforcer: report-only as claimed? Can it crash the design phase on a missing ledger?
4. deep-run.js: secrets hygiene (no key values printed/written), quota-probe error classification correctness (rate_limit vs insufficient_quota), partial-provider failure path, injectable seam actually used by all transport calls (no stray fetch bypass).
5. checkpoint.js normalizeExisting: lossy? (must not drop unknown fields silently or corrupt resume state).
...
Only blocker/major flip the verdict. file:line specific, no style nits.
...
+        `Linked: \`H-1\`, \`${rId}\`.`,
         ...
         `INPUTS: see \`inputs.md\`.`,
         `TRACE: see \`trace.md\`.`,
@@ -84,6 +90,100 @@ function buildGranularStoriesBody(candidates, outcome) {
     .join("\n");
 }
