# QA/INTEGRITY REVIEW — SP-20260610-002 Lane B (dispatch/registry coherence), canonical WarpOS

You are reviewing an INTEGRATED diff (2 build units, base 75cd7e5) fixing four verified bugs:
- WG-2: 6 agent specs had `model: inherit` (session-model leak vs role-registry routing SoT); beta.md pinned sonnet vs registry opus. Fix: pins + frontmatter-guide rewrite + role-parity-scan model check.
- WG-5: dispatch-contract class_derivation first-match {tier:lead}->manager forced in-process shape for design-lead while role-registry routes it openai/gpt-5.5 subprocess. Fix: cross_provider_consult_lead rule inserted BEFORE the generic lead rule + shape-vs-route parity check.
- WG-4: epsilon.md documented an Agent-tool conduct loop a teammate-spawned epsilon cannot execute (harness: "Agent is not available inside subagents", ED-041). Fix: conduct-routes-by-spawn-context section + startup route self-check; dispatch-guide section added.
- WG-6: teammate-epsilon has no wake event for its own background subprocess completions -> infinite silent stalls (observed x3). Fix: stall rules in epsilon.md + NEW fail-closed scripts/checks/epsilon-liveness.js (+7-case fixture test) wired report-only into scan:full.

QA LENS — beyond the backend attack list, also check INTEGRITY: do the claims in commit messages/test names match what the code actually does? Planted-fixture tests must really bite (would the test fail if the check were broken?). Trace: do the 4 fixes map to the 5 tickets T-287..291?
ATTACK THESE SPECIFICALLY:
1. Can role-parity-scan's new model-pin check FALSE-GREEN? (e.g. roles whose spec path is missing/unparseable frontmatter — does it fail loud or skip silent?)
2. Is the dispatch-contract rule insertion ORDER-SAFE? (first-match list — does any OTHER role match the new rule unintentionally? claude leads must still derive manager.)
3. Can epsilon-liveness LIE? (malformed ledger, empty dirs, timezone/mtime edge, --now misuse; it must fail-closed on unreadable-ledger-with-evidence, exit 0 only honestly.)
4. Doc consistency: epsilon.md vs agent-dispatch-guide.md vs the mode:sprint skill ED-041 language — any contradiction left?
5. Pins correctness: every pinned model value must equal role-registry.json's model for claude-provider roles; claude-opus-4-8 fallback pin for cross-provider roles (cabinet, ops-analyst).
...
Only blockers/major flip the verdict to FAIL. Be specific (file:line), no style nits.
...
@@ -415,6 +418,129 @@ function evaluateRegistry(reg, catalog) {
   return errors;
 }
