FINAL security verdict on a subprocess reaper signature matcher. The prior cycle's remaining HIGH was node entry-mode-switch flags (`--test=x`, `--run=foo`) in =value form before the wrapper still being treated as having a script operand. FINAL FIX: nodeScriptOperand no longer parses node's flag grammar at all. It requires the wrapper to be argv[1] — the FIRST argument after `node`; if argv[1] starts with `-` (ANY flag), there is NO operand we trust (return null => no match). This matches the verified invariant that real WarpOS dispatch is ALWAYS `node scripts/dispatch-{claude,agent}.js <role> <prompt>` (no node flag before the script). The operand must then be absolute + path.resolve()'d to equal a canonical wrapper path.

Verify this CLOSES the entry-mode-switch HIGH and the whole flag class: `node --test=x <w>`, `node --run=foo <w>`, `node --import <w>`, `node --eval=<code> <w>`, `node -e <code> <w>`, `node --max-old-space-size=4096 <w>`, `node --anything <w>` ALL no-match (a flag is in the argv[1] slot); ONLY `node <abs-wrapper> …` matches. Is there ANY residual false-KILL path in the signature matcher now? Re-confirm CRIT-1/2/3, HIGH-4, MED-7/8, /proc argv-array, fs-required still hold. Output VERDICT: PASS or FAIL + findings tagged CRITICAL/HIGH/MEDIUM/LOW. PASS iff no CRITICAL/HIGH false-kill path remains.
