You are a security reviewer. Review ONE file — a subprocess REAPER that can terminate OS processes — for security defects. This is a privilege surface: a mis-scoped reaper could kill a live, legitimate process and destroy uncommitted work. Render a BINDING verdict: PASS or FAIL.

CONTEXT: WarpOS dispatches AI agents as OS subprocesses (`node scripts/dispatch-claude.js` / `dispatch-agent.js`, which spawn `claude`/`codex`/`gemini` CLIs). When the Claude Code harness reaps the wrapper, the OS can leave the grandchild provider CLI running with no completion record (an orphan holding a model session + memory). This reaper detects + (with --apply) SIGTERM-terminates those orphans. It runs report-only on session start and via /warp:health.

THE SAFETY CONTRACT (what the reaper MUST guarantee): a process is terminated ONLY when ALL hold — (1) its command line matches a WarpOS dispatch signature; (2) it is orphaned (parent dead or reparented to init, ppid<=1); (3) it is older than ~20min; (4) it holds NO fresh concurrency lock; (5) it is NOT the reaper's own process tree. Any ambiguity => SKIP (reap nothing). FAIL-OPEN: any enumeration/parse error => reap nothing. SIGTERM-first (never SIGKILL-first). Report-only by default; --apply required to terminate.

EVALUATE specifically:
- Can any code path terminate a process that does NOT meet ALL five gates? (command-injection via crafted cmdline, a signature false-match on a foreign process, an own-tree-exclusion bypass, a races where ppid is reused.)
- Is the fail-open real (does an exception anywhere lead to "reap nothing", never "reap everything")?
- Is `terminate()` safe (no shell injection via pid; SIGTERM not SIGKILL; Windows taskkill without /F)?
- Is the orphan determination sound (ppid<=1 OR parent-not-alive; EPERM treated as alive = conservative)?
- Could the process enumeration (PowerShell CIM / ps) be subverted to inject a fake "orphan"?
- Any TOCTOU between classify and terminate that could kill a freshly-spawned legit process reusing a PID?

Output your verdict as: VERDICT: PASS or VERDICT: FAIL, then a bullet list of findings each tagged [CRITICAL]/[HIGH]/[MEDIUM]/[LOW], each with the specific line/mechanism and a concrete fix. If PASS, still list any LOW/MEDIUM hardening notes. Be rigorous — this kills processes.

=== FILE: scripts/dispatch/reap-orphans.js ===
#!/usr/bin/env node
"use strict";
