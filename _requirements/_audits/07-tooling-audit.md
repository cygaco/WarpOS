...
| 2 | MEDIUM | `ADMIN_SECRET` used by /api/credits/grant but undocumented | .env.local.example | Add to env example |
| 3 | MEDIUM | Stripe price ID names reconciled to STARTER/PRO/SCALE — verify no stale fallback aliases remain | stripe/checkout route | Use the unified STARTER/PRO/SCALE names only |
...
3. **Standardize Stripe price ID names** — unify on STARTER/PRO/SCALE, eliminate stale fallback aliases (1 finding)
