...
If builders ran now: (1) /api/test would ship with client-discoverable gate value, (2) OAuth callbacks would be vulnerable to state replay, (3) no security event logging means incidents go undetected, (4) Stripe webhook could grant credits to wrong user.
...
| # | Category | Finding | Location | Fix |
|---|---|---|---|---|
| 4 | A01 | No per-user rate limits; no role-based access for billable prompts | /api/claude, /api/launch-jobs | Add per-user rate limits (10/min), role checks for BILLABLE_PROMPTS |
...
| 9 | A04 | Daily credit spend limit (500/day per user) specified but not enforced | /api/claude, /api/credits/debit | Add per-user daily counter in Redis |
| 10 | A04 | Concurrent credit debit race condition in in-memory fallback (local dev) | src/lib/credits.ts lines 183-190 | Require Redis in production; mark in-memory as dev-only |
...
| 13 | A04 | No abuse detection for anonymous endpoints (EXTRACT/PROFILE can be called without auth) | /api/claude | Spec future abuse detection story |

### Spec Gaps

| # | Finding | Fix |
|---|---|---|
| SG-1 | No security AC on credit-spending stories (SEGMENT_PLAN, CHANNEL_ASSETS, LAUNCH_RESEARCH_PREP rerun) | Add 402 insufficient-balance AC to all billable stories |
| SG-2 | No CORS configuration documentation | Document CORS policy in SECURITY.md |
| SG-3 | No security checklist for Launch Console client DOM interaction | Add security checklist to LAUNCH_CONSOLE_SPEC.md |
...
5. **Add security AC to billable stories** — ensure all credit-spending operations have 402 acceptance criteria (1 finding)
