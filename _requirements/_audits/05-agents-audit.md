...
If we launched a regen run now: (1) Builders reading AGENTS.md would not see utils.ts and prompts.ts as foundation-owned, risking accidental modifications. (2) Orchestrator would fail at Phase 4+ gate checks because scripts/validate-gates.js doesn't exist. (3) PlanPage coordination rule (deep-dive-qa + scope-curation must not be parallel) has no automated enforcement.
...
| 3 | MEDIUM | Phase 4+5 PlanPage coordination rules documented but no automated enforcement | Add store field tracking dispatch mode; add check in orchestrator prompt |
...
| 8 | MEDIUM | LaunchReadinessScore "uncapped" scoring (segment asset packs 25%) documented but tiers should be verified against code | Verify launch-readiness.ts tier definitions handle >100 |
...
| 13 | MEDIUM | PROMPT-TEMPLATES.md uses {{FEATURE_DIR}} but credits→credits-economy mapping only documented in TASK-MANIFEST | Add mapping note to ORCHESTRATOR-PROMPT |
...
2. **Create scripts/validate-gates.js** or remove reference from ORCHESTRATOR-PROMPT — orchestrator will fail at phase advancement (1 finding)
