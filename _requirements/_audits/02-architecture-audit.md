...
If agents used these docs as-is, the top risks are: (1) Field name mismatch `searchQueries` vs `generatedQueries` will cause data wiring failures in launch-research builders, (2) missing `PROMPTS.md` reference will leave agents unable to find Launch Console prompt structure, and (3) two-phase launch-research pipeline not fully described in FLOW_SPEC will cause single-phase implementations.
...
|---|---|---|---|---|
| 1 | MEDIUM | VALIDATION_RULES.md | Missing Step 2 constraints validation (direction, budget, channel types) | Add form validation rules for all constraint substeps |
...
|---|---|---|---|---|---|
| 1 | CRITICAL | FLOW_SPEC.md (line 73) | launch-research/STORIES.md GS-RES-01 | `searchQueries` vs `generatedQueries` — different field names for same data | Standardize on `generatedQueries` (matches types.ts); update FLOW_SPEC |
...
| 3 | CRITICAL | FLOW_SPEC.md (line 82) | launch-research/STORIES.md GS-RES-11 | Step 5 says "Run two-phase launch-research pipeline" but doesn't mention `launchResearchPrep` intermediate field | Update FLOW_SPEC to show both pipeline phases and field names |
| 4 | HIGH | FLOW_SPEC.md entry states | plan-generation/STORIES.md | Different entry state vocabulary ("Has master launch narrative" vs "Fresh"/"Async-complete"/"Returning") | Add standard vocabulary definition to FLOW_SPEC or AGENT_GUIDE |
| 5 | HIGH | COMPONENT_HIERARCHY.md | src/components/steps/ | Component naming debt: Step10AssetPacks = Step 8, Step11Channels = Step 9, etc. | Add explicit "Component File column is source of truth" note |
...
| 2 | HIGH | DevConsole outermost-wrapper constraint missing from COMPONENT_HIERARCHY.md | Agents might refactor it as sibling | Add bolded constraint note |
...
| 2 | MEDIUM | FLOW_SPEC.md | Two-phase launch-research pipeline not fully spelled out | Add field names and intermediate step |
| 3 | MEDIUM | DATA-CONTRACTS.md | Requires reading 10 external files to verify wires | Add master summary table |
| 4 | LOW | DESIGN_TOKENS.md | Launch Console tokens use slightly different values than app | Document intentional divergence |
...
3. **Update FLOW_SPEC Step 5** to show two-phase pipeline with `launchResearchPrep` → `launchResearchReport` field names (1 finding)
