# AcmeLaunch — Master Copy Strategy

This document defines the non-negotiable principles that govern **all in-app copy** at AcmeLaunch.

It applies to:

- Human-written copy
- AI-generated copy
- Rewrites, edits, and variants

If there is a conflict between documents, **this document wins**.

---

## Purpose

The purpose of in-app copy at AcmeLaunch is to:

- Help users understand what is happening immediately
- Reduce anxiety around launching
- Guide users toward the next meaningful action
- Do so in a way that feels competent and direct, not corporate or patronizing
- Reflect AcmeLaunch's personality without sacrificing clarity

AcmeLaunch copy is not marketing copy.
AcmeLaunch copy is not documentation.
AcmeLaunch copy is a **product interface**.

---

## Audience

Primary audience:

- Founders under pressure (preparing a first launch, relaunch, or paid offer with limited time)
- Mix of technical and non-technical founders
- Range from first-time founders to experienced operators
- Mobile and desktop users

Implications:

- Copy must be readable at a standard comprehension level
- Users are stressed — they need reassurance, not pressure
- Users are goal-oriented — they want to get things done, not read explanations
- If copy feels condescending, users disengage
- If copy feels confusing, users lose trust in the AI outputs

---

## Core Philosophy

### 1. Clarity Comes First

If a line of copy cannot be understood on first glance, it is wrong.

This rule overrides:

- Humor
- Brand personality
- Cleverness
- Brevity (if brevity creates ambiguity)

If clarity and personality ever conflict, **clarity wins**.

### 2. One Idea Per Unit

Each unit of copy communicates **one idea only**.

A "unit" is:

- A button
- A heading
- A progress label
- A loading message
- A tooltip
- A toast notification
- An error message

If a line requires "and," "but," or multiple clauses, it should be split.

### 3. Reduce Anxiety, Don't Create It

Launching is inherently stressful. AcmeLaunch copy must never add to that stress.

Therefore:

- Reassurance beats urgency
- Progress indicators beat uncertainty
- Honest status beats vague promises
- Calm confidence beats hype

Copy should make action feel:

- Safe (nothing irreversible without warning)
- Progressive (each step builds on the last)
- Worth it (the output justifies the input)

#### 3a. Do Not Undercut Momentum

Reducing anxiety does not mean removing confidence.

When the product signal is strong (e.g., landscape data loaded, plan generated, score increased), copy should:

- State what happened clearly
- Invite the next step
- Avoid apology or hesitation language

Avoid phrases that:

- Disclaim results ("this is just a suggestion")
- Pre-empt rejection ("you can always change this later" on every screen)
- Soften an already low-risk action

The goal is calm confidence, not reassurance-by-withdrawal.

### 4. Personality Is a Tool, Not the Goal

AcmeLaunch has a direct, competent personality — like a seasoned launch operator who names the next concrete action.

However:

- Personality exists to **help the user**, not entertain itself
- The phase names (PLAN/PREP/LAUNCH) are structural, not decorative
- Credit imagery is functional (the currency), not whimsical

If removing personality does not reduce effectiveness, it should be removed.

---

## Priority Order (Critical)

All copy decisions must follow this order:

1. **UX best practices**
   - Clarity
   - Scannability
   - Accessibility
   - Conversion

2. **Audience constraints**
   - Stress level
   - Goal orientation
   - Trust sensitivity
   - Varied technical literacy

3. **Surface-specific rules**
   - CTA vs modal vs toast vs tooltip
   - Stakes of the moment

4. **AcmeLaunch personality**
   - Direct, competent tone
   - Structural phase framing (sparingly)
   - Credit imagery (where relevant)

AcmeLaunch personality may **never override** items 1–3.

---

## What "Good Copy" Means at AcmeLaunch

Good AcmeLaunch copy is:

- Immediately understandable
- Shorter than expected
- Honest about what's happening
- Clear about what happens next
- Supportive without being soft
- Confident without being loud

Good copy does **not**:

- Explain the product unnecessarily
- Try to be clever at the expense of meaning
- Sound like marketing
- Sound like a chatbot trying to be your friend
- Use launch jargon the user doesn't know

---

## Conversion Philosophy

At AcmeLaunch, conversion does not mean pressure.

Conversion means:

- Helping users take the next step they already want to take
- Making that step feel easy and low-risk
- Removing uncertainty, not adding urgency

AcmeLaunch optimizes for:

- Completing the current step
- Moving forward in the wizard
- Generating one more output
- Coming back to finish later

AcmeLaunch does **not** optimize for:

- Forcing commitment
- Artificial urgency
- FOMO-based pressure ("others are launching NOW")
- Loud hype language

Copy should make users think:

> "Alright, let's do the next one."

Not:

> "I need to hurry before I miss out."

---

## When NOT to Be Playful

There are moments where personality actively harms trust.

In these contexts, copy must be neutral and direct:

- Error messages (API failures, extraction errors)
- Payment and credit transactions
- Authentication and account access
- Data privacy and export
- Destructive or irreversible actions (data clearing, session reset)
- Rate limiting or budget exhaustion messages

In these moments:

- Use full sentences
- Use plain language
- Do not joke
- Do not reference credits metaphorically
- Do not soften seriousness with humor

Trust > tone.

---

## Emotional Safety

AcmeLaunch copy must never:

- Shame the user
- Blame the user
- Imply incompetence
- Create guilt about launch progress
- Suggest the user is behind or failing

Avoid phrasing like:

- "You failed to…"
- "Your plan is missing…"
- "You should have…"
- "Other founders are…"

Prefer phrasing like:

- "That didn't work."
- "We didn't find any results."
- "Try again with different terms."
- "You can update this anytime."

The app is always on the user's side.

---

## Success Criteria for Copy

A piece of copy is successful if:

1. A founder under pressure can understand it instantly
2. The next step is obvious
3. The user does not feel judged or pressured
4. The copy feels intentional and competent
5. Removing personality would not break meaning

If any of these are false, the copy should be revised.

---

## AI Generation Rules

This strategy applies equally to:

- Human-written copy
- AI-generated copy (prompts, loading messages, error text)
- Edits, rewrites, and variants

The AI must:

- Default to clarity and brevity
- Apply best practices before personality
- Choose the correct surface rules automatically
- Refuse or redirect requests that violate this strategy

---

## Strategy Enforcement

This document is enforced by:

- Copy Mechanics & Grammar Rules
- Surface-Specific Writing Rules
- Copy Surface Map

If a future document contradicts this strategy, this strategy takes precedence.

---

## Final Principle

AcmeLaunch copy should feel:

- Easy to read
- Easy to trust
- Easy to act on

Directness is welcome.
Clarity is required.
