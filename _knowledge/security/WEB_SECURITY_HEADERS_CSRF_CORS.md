...
> **Coverage note for the gauntlet:** presence/absence rules ([WEBSEC-01..03, 06]) are largely machine-detectable — high precision. The *correctness* qualifiers (CSP strength, HSTS `max-age`, exact-vs-reflecting CORS, JWT verification details, cookie-vs-bearer CSRF applicability, cooldown policy) are judgment-shaped — written as assertions so a reasoning reviewer can read the *value and auth model*, not just the header name, at build time.
