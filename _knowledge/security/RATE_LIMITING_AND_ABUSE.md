...
> **Coverage note:** RATE-06/07/10 are largely machine-detectable (presence of `max_tokens`, 429 handling, limiter config). RATE-01/02/03/04/09 require reading the endpoint→enforcement path (is the quota *actually* checked server-side? is the counter account-keyed?) and are judgment checks — written as independent assertions a reasoning reviewer can evaluate.
