...
function describeSpawnFailure(r) {
  ...
  // `manifest.warpos.version matches version.json` FAIL as an adjacent OK "agent under
...
const NORMALIZE_ALLOWED_RELPATHS = new Map([
  // NOTE (β (a) ruling): .claude/framework-installed.json and _warpos/MANIFEST.json are NOT normalized here —
...
const ACCEPT_ONLY_IN_UPGRADED_3C = [
  ...
  { re: /^_warpos\/agents\/president\/_system\/policy\/decision-policy\.md$/, why: "a _warpos source-mirror policy copy committed into the frozen N-1 tag; not in the fresh-N install set" },
...
const CONTENT_PRESERVE_DIVERGENCE_3C = new Map([
  ...
  [
    ".claude/manifest.json",
    "PROJECT-OWNED identity card (β b088 ownership discriminator). Its FRAMEWORK-owned field warpos.version is verified elsewhere (3b scan:install version-match, and restamped by the update); its PROJECT-owned config (providers/agentProviders/features) is PRESERVED across upgrade (same principle as the restamp touching ONLY warpos.version) while fresh-N gets 1.0.0 defaults; project.name/slug is the install-dir basename. NOTE: a FUTURE framework-owned manifest field would need separate coverage (it is not caught here).",
  ],
...
function compareTreeContents(treeA, treeB, commonRelPaths, ctx) {
  ...
  for (const rel of commonRelPaths) {
    ...
    // _warpos/MANIFEST.json (whole-tree ownership inventory) and .claude/framework-installed.json (per-install
...
// _warpos/MANIFEST.json and .claude/framework-installed.json are per-install DERIVED records that
...
// one of those existing ruled dispositions (or the documented _warpos/MANIFEST.json walk self-reference).
...
// _warpos/MANIFEST.json is a WHOLE-TREE ownership inventory (manifest/build.js product-mode enumerates every
...
function isDispositionedInventoryKey(key) {
  return (
    key === "_warpos/MANIFEST.json" || // walk self-reference: the manifest lists itself only when it
...
function compareWarposManifestInventory(treeA, treeB, rel) {
  let a, b;
  try { a = readJsonRecord(treeA, rel); } catch (e) { return { equal: false, reason: `_warpos/MANIFEST.json unparseable in upgraded tree: ${e.message}` }; }
  try { b = readJsonRecord(treeB, rel); } catch (e) { return { equal: false, reason: `_warpos/MANIFEST.json unparseable in fresh-N tree: ${e.message}` }; }
  ...
  if (JSON.stringify(topA) !== JSON.stringify(topB)) {
    return { equal: false, reason: "_warpos/MANIFEST.json top-level fields diverge (warposVersion/schema/version/…)" };
  }
  const pa = (a && a.paths) || {};
  const pb = (b && b.paths) || {};
  const bad = [];
  ...
  if (bad.length) {
    return { equal: false, reason: `_warpos/MANIFEST.json framework-owned inventory entries diverge (${bad.length}): ${bad.slice(0, 5).join(", ")}` };
  }
...
// LOAD-BEARING installedVersion check F3 requires (3b checks manifest.warpos.version, a DIFFERENT field, so
...
function selfTest() {
  ...
  t("ED-262 REGRESSION-DETECTING: a scan failure whose FAIL line sits >400 chars from the END is still NAMED (the old slice(-400) tail-truncation MASKED it as an adjacent OK line — the whole 'president-missing' triage detour)", () => {
    const failLine = "FAIL  manifest.warpos.version matches version.json  (manifest=0.17.0 version.json=1.0.0)";
    ...
    return desc.includes("manifest.warpos.version matches version.json") && desc.startsWith("code=1 FAILING:");
  });
  ...
  // NORMALIZE_ALLOWED_RELPATHS; framework-installed.json / _warpos/MANIFEST.json
  ...
  t("β(a) framework-installed structured: an UNDISPOSITIONED framework field (pathRegistryVersion) divergence REDs", () => {
    const dirA = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-fi-owned-red-a-"));
    const dirB = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-fi-owned-red-b-"));
    try {
      fs.mkdirSync(path.join(dirA, ".claude"), { recursive: true });
      fs.mkdirSync(path.join(dirB, ".claude"), { recursive: true });
  ...
  t("β(a) framework-installed structured: per-install fields (source/installedCommit/assets) differ but framework-owned fields match → EQUAL (filtered, NOT an escape of a framework field)", () => {
    const dirA = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-fi-perinstall-a-"));
    const dirB = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-fi-perinstall-b-"));
    try {
      fs.mkdirSync(path.join(dirA, ".claude"), { recursive: true });
      fs.mkdirSync(path.join(dirB, ".claude"), { recursive: true });
      const owned = { $schema: "warpos/framework-installed/v2", installedVersion: "1.0.0", pathRegistryVersion: "v5", manifestSchema: "warpos/framework-manifest/v2", generated: [".claude/paths.json"] };
  ...
  t("β(a) _warpos/MANIFEST inventory structured: an UNDISPOSITIONED framework inventory entry (scripts/foo.js) with a divergent sha REDs", () => {
    const dirA = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-red-a-"));
    const dirB = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-red-b-"));
    try {
      fs.mkdirSync(path.join(dirA, "_warpos"), { recursive: true });
      fs.mkdirSync(path.join(dirB, "_warpos"), { recursive: true });
      const base = { $schema: "warpos/manifest/v1", warposVersion: "1.0.0" };
      fs.writeFileSync(path.join(dirA, "_warpos", "MANIFEST.json"), JSON.stringify({ ...base, generatedAt: "2026-07-21T00:00:00.000Z", paths: { "scripts/foo.js": { owner: "framework", sha256: "AAA" } } }));
      fs.writeFileSync(path.join(dirB, "_warpos", "MANIFEST.json"), JSON.stringify({ ...base, generatedAt: "2026-07-21T00:09:00.000Z", paths: { "scripts/foo.js": { owner: "framework", sha256: "BBB" } } }));
      const ctx = { sandboxRoots: [dirA, dirB], nonces: [] };
      return compareTreeContents(dirA, dirB, ["_warpos/MANIFEST.json"], ctx).equal === false;
    } finally {
      fs.rmSync(dirA, { recursive: true, force: true });
      fs.rmSync(dirB, { recursive: true, force: true });
    }
  });
  t("β(a) _warpos/MANIFEST inventory structured: DISPOSITIONED entries (a frozen-accept key + a runtime events key) diverge but are FILTERED → EQUAL (generatedAt dropped)", () => {
    const dirA = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-green-a-"));
    const dirB = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-green-b-"));
    try {
      fs.mkdirSync(path.join(dirA, "_warpos"), { recursive: true });
      fs.mkdirSync(path.join(dirB, "_warpos"), { recursive: true });
      const base = { $schema: "warpos/manifest/v1", warposVersion: "1.0.0" };
      ...
      fs.writeFileSync(path.join(dirA, "_warpos", "MANIFEST.json"), JSON.stringify({ ...base, generatedAt: "2026-07-21T00:00:00.000Z", paths: pa }));
      fs.writeFileSync(path.join(dirB, "_warpos", "MANIFEST.json"), JSON.stringify({ ...base, generatedAt: "2026-07-21T00:09:00.000Z", paths: pb }));
      const ctx = { sandboxRoots: [dirA, dirB], nonces: [] };
      return compareTreeContents(dirA, dirB, ["_warpos/MANIFEST.json"], ctx).equal === true;
    } finally {
      fs.rmSync(dirA, { recursive: true, force: true });
      fs.rmSync(dirB, { recursive: true, force: true });
    }
  });
  t("β(a) _warpos/MANIFEST inventory structured: a top-level framework field (warposVersion) divergence REDs (not filtered)", () => {
    const dirA = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-top-a-"));
    const dirB = fs.mkdtempSync(path.join(os.tmpdir(), "warpos-gateb-selftest-wm-top-b-"));
    try {
      fs.mkdirSync(path.join(dirA, "_warpos"), { recursive: true });
      fs.mkdirSync(path.join(dirB, "_warpos"), { recursive: true });
      fs.writeFileSync(path.join(dirA, "_warpos", "MANIFEST.json"), JSON.stringify({ $schema: "warpos/manifest/v1", warposVersion: "1.0.0", generatedAt: "2026-07-21T00:00:00.000Z", paths: {} }));
      fs.writeFileSync(path.join(dirB, "_warpos", "MANIFEST.json"), JSON.stringify({ $schema: "warpos/manifest/v1", warposVersion: "0.17.0", generatedAt: "2026-07-21T00:09:00.000Z", paths: {} }));
      const ctx = { sandboxRoots: [dirA, dirB], nonces: [] };
      return compareTreeContents(dirA, dirB, ["_warpos/MANIFEST.json"], ctx).equal === false;
    } finally {
      fs.rmSync(dirA, { recursive: true, force: true });
