 ...
 *
 * Orchestrator for the /product:bootstrap skill.
 *
 * Inputs (CLI):
...
function paragraphsToHtml(text) {
  // Split on blank lines, escape, wrap in <p>
  const blocks = String(text).split(/\n{2,}/);
  return blocks
    .map((b) => {
      const trimmed = b.trim();
      if (!trimmed) return "";
      // Detect bulleted list (lines starting with -, *, •)
      ...
      if (allBullets && lines.length > 1) {
        return (
          ...
          lines
            .map(
              (l) =>
                "<li>" + htmlEscape(l.replace(/^\s*[-*•]\s+/, "")) + "</li>",
            )
            .join("") +
          "</ul>"
        );
      ...
      if (allNum && lines.length > 1) {
        return (
          ...
          lines
            .map(
              (l) =>
                "<li>" + htmlEscape(l.replace(/^\s*\d+[.)]\s+/, "")) + "</li>",
            )
            .join("") +
          "</ol>"
        );
      }
      return "<p>" + htmlEscape(trimmed).replace(/\n/g, "<br>") + "</p>";
    })
    .filter(Boolean)
    .join("\n  ");
}
...
function printHelp() {
  process.stdout.write(
    [
      ...
      "",
      "Usage: /product:bootstrap [--slug <name>] [--section-set minimal|extended] " +
  ...
  if (!args.answersFile) {
    process.stderr.write(
      [
        ...
        "",
        "/product:bootstrap is intended to be invoked through the Claude",
