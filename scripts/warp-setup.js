...
// Relationship maps baseline (hooks, skills, memory, systems-inventory, tools).
// These get regenerated by /maps:all but users need a non-empty baseline so
// /maps:systems and /check:system have something to diff against on day one.
const mapsSrc = path.join(WARPOS, ".claude/project/maps");
if (fs.existsSync(mapsSrc)) {
  const mapsCount = copyDir(mapsSrc, path.join(TARGET, ".claude/project/maps"));
  if (mapsCount > 0) {
    log(
      "ok",
      `Relationship maps: ${mapsCount} files installed (regenerate with /maps:all)`,
    );
    installed += mapsCount;
  }
}

// Requirements templates (the product-spec skeleton users fill in).
// 10 numbered subdirs: 00-canonical, 01-design-system, 02-copy-system,
// 03-requirement-standards, 04-architecture, 05-features, 06-operations,
// 07-security, 08-testing, 09-automation.
// Plus _example-onboarding feature showing the shape users should copy.
const reqSrc = path.join(WARPOS, "requirements");
if (fs.existsSync(reqSrc)) {
  const reqCount = copyDir(reqSrc, path.join(TARGET, "requirements"));
  if (reqCount > 0) {
    log(
      "ok",
      `Requirements templates: ${reqCount} files installed (ask Alex to help fill them in)`,
    );
    installed += reqCount;
  }
}

// Engineering patterns library (battle-tested recipes: AI orchestration,
// rate limiting, encrypted storage, UI component kit, background API).
const patternsSrc = path.join(WARPOS, "patterns");
if (fs.existsSync(patternsSrc)) {
  const patternsCount = copyDir(patternsSrc, path.join(TARGET, "patterns"));
  if (patternsCount > 0) {
    log("ok", `Engineering patterns: ${patternsCount} files installed`);
    installed += patternsCount;
  }
}
