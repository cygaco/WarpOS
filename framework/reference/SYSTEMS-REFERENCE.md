...
Built on Claude Code. Project: consumer product (job search app). Branch: skeleton-test6.
...
### .claude/memory/learnings.jsonl — 36 entries, target 30-50
...
### .claude/memory/traces.jsonl — EMPTY (reasoning engine unused)
...
### .claude/memory/modifications.jsonl — 11 entries, all validated
...
### .claude/memory/systems.jsonl — 22 entries
...
### .claude/memory/inbox.jsonl — 2 messages, 24h TTL
...
### .claude/handoffs/*.md — 20 files (pruned from 147)
...
### .claude/.sleep-journal.md — 3 entries (Apr 1-4)

```
WRITES:  /sleep:deep  (all 6 phases append results)
         /sleep:quick (phases 1-2 only)
READS:   context-enhancer.js  (surfaces key findings)
         /wakeup:quick        (morning briefing source)
FORMAT:  Markdown sections: NREM, Cleanup, Replay, REM Dreams, Repair,
         Growth — one entry per sleep cycle
NOTES:   Append-only. Never deleted, only grows.
```

### .claude/.sleep-coaching.md — Current: 2026-04-04

```
WRITES:  /sleep:deep Phase 6  (pre-computed session briefing)
READS:   session-start.js     (morning briefing at session start)
         /wakeup:quick        (espresso briefing)
FORMAT:  Markdown: Morning Briefing, Suggested First Task, System Health,
         Meta-Patterns
NOTES:   Overwritten each sleep. "Sleep-time compute" concept from
         Google 2025 — use idle time for pre-computation.
```

### .claude/.sleep-dreams.md — Current: 2026-04-04

```
WRITES:  /sleep:deep Phase 4  (REM dreaming — speculative solutions)
READS:   /wakeup:dreams       (review overnight dream solutions)
FORMAT:  Markdown: problem + solution per dream, with confidence level
NOTES:   Speculative. Tagged as "dream", not decisions. Overwritten
         each sleep.
```
...
The directory `.claude/memory/` containing all 5 JSONL files (learnings,
...
`.claude/handoffs/`. Also broadcasts to inbox on session end for
...
`.claude/memory/modifications.jsonl` tracks changes to Alex's own
...
Inbox system (`.claude/memory/inbox.jsonl`) for multiple Alex sessions to
...
4 skills (`/retro:full`, `/retro:meta`, `/retro:code`, `/retro:git`) for
cross-run pattern analysis. Designed to produce BUGS.md, HYGIENE.md,
LEARNINGS.md. Never executed in current form.
...
SLEEP CYCLE (manual: /sleep:deep)
     ...
     +-- Phase 3 (Replay) ---> .sleep-journal.md (insights)
     +-- Phase 4 (REM) ------> .sleep-dreams.md (speculative solutions)
     +-- Phase 5 (Repair) ---> systems.jsonl (mark broken), npm audit
     +-- Phase 6 (Growth) ---> .sleep-coaching.md (next-session briefing)
```
