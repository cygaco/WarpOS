...
1. Tag your current state: `git tag pre-warpos-0.15.4-update HEAD`.
2. Confirm clean working tree: `git status --porcelain` empty.

## Run the update

```bash
node scripts/warpos/update.js --to 0.15.4 \
  --source ../WarpOS \
  --target . \
  --dry-run
...
```bash
git reset --hard pre-warpos-0.15.4-update
```
