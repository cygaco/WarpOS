...
- `--refresh` — re-scan all skills and rebuild `.claude/maps/skills.jsonl` + `.claude/maps/skills.md`
- `--terminal` — render as ASCII art (default)
- `--file` — write to `.claude/maps/skills.txt`
- `--html` — write styled HTML to `.claude/maps/skills.html`
...
If `.claude/maps/skills.jsonl` exists and `--refresh` not passed → skip to Step 5 (render).
...
1. Extract outbound references — grep for:
   ...
   - `.claude/memory/` — data store references
...
### Step 3: Generate `.claude/maps/skills.jsonl` + `.claude/maps/skills.md`

Write JSONL with `_meta` header (follow `.claude/maps/enforcements.jsonl` pattern), then one entry per skill.
...
After writing output, clear the `skills` entry from `.claude/maps/.stale.json` if it exists:
