...
Writes `.claude/.session-checkpoint.json` with:
...
Write to `.claude/.session-checkpoint.json` and confirm:
...
Also update `.claude/.last-checkpoint` timestamp so the periodic checkpoint timer resets.
