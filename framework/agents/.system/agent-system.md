...
> The specification is the product. Code is disposable. The quality gate is automated — no human reviews code. If you are adding human code review steps, you are breaking the architecture. See docs/99-resources/research/ for the taxonomy.
...
→ δ runs gauntlet → manages fix cycles → auditor analysis
...
interface Heartbeat {
  ...
  phase: number | string;
  feature: string;
  agent: string;       // "alpha" | "beta" | "gamma" | "delta" | "auditor" | "builder-*" | "evaluator" | "security" | "compliance" | "qa" | "fix-*"
  status: string;
  cycleStep: string;
...
- **Gamma** (Alex γ) handles adhoc builds — single features during development. See `.claude/agents/alex/gamma.md`.
...
- **Delta** (Alex δ) handles oneshot builds — full skeleton runs with state machine, cycles, points. See `.claude/agents/alex/delta.md`.
...
- You hand results to the evaluator and security agents.
- If evaluator flags issues, you re-dispatch with the evaluator's feedback attached.
...
- You analyze patterns across the bug dataset, conflict dataset, evaluator reports, and security reports.
...
4. Check evaluator reports — any repeated failures on same criteria? Determine: is the spec ambiguous (patch spec), is acceptance criteria wrong (patch criteria), or is the agent struggling (add constraints to prompt template)?
...
- You run independently — you do not depend on evaluator results.
...
- You do NOT evaluate code quality (evaluator's job) or security (security agent's job).
- You do NOT fix code or suggest fixes.
- You do NOT communicate with the user — ever.
- Full spec: `.claude/agents/.system/oneshot/compliance.md`
...
- **Analyze mode** (personas 8-13): deep tracing — flow tracer (ASCII + Mermaid diagrams), data flow tracker, state snapshot differ, timing analyzer, contract verifier, lifecycle auditor.
- You are self-orchestrating — gamma dispatches you as one agent, you handle the parallelism internally.
- You make pass/fail decisions only. You do NOT fix code.
- You do NOT communicate with the user — ever.
- Full persona definitions: `{mode}/personas.md` (QA Orchestrator, QA-Scan, QA-Analyze templates).

### Fix Agent

- You are spawned with a specific issue from evaluator or security output.
...
5. **WAIT GATE:** Orchestrator MUST wait for ALL THREE reviewers (evaluator, compliance, security) to return results before proceeding. Do NOT dispatch fix agents while any reviewer is still running. Once all three complete: if any fail, merge into unified fix brief → dispatch fix agent
...
- If an agent produces bad output, let it finish and fail. The evaluator catches it. The fix cycle handles it.
- The lead's power is shaping the environment for the NEXT cycle, not interrupting the current one.
- After the auditor adjusts rules/spec/lint: the next builder works in a stricter environment. That is the improvement mechanism.
...
Security doesn't modify code — it's read-only. Once a feature passes the evaluator + compliance gate, its builders are done and can't regress. Security can scan in the background while the next phase's builders start.
...
**Rule**: Builders in the next phase may start as soon as their dependencies are **eval-gated** (evaluator + compliance pass). They do NOT need to wait for security. Security failures spawn fix agents that run alongside Phase 3 builders — separate file scopes, no conflict.
...
#### Overlap 4: Multi-feature evaluator fan-out

When multiple builders finish in a phase, each feature's evaluator review is independent. Spawn one evaluator per feature in parallel instead of reviewing them one at a time.
...
**Rule**: Per-feature reviews (evaluator, compliance) always fan out. Security is a single codebase-wide scan, so it runs once per cycle.
...
| `claude` | Anthropic's Claude Code | Builders, evaluator, security |
...
**Hard rule: maximum 3 fix attempts per unified brief. After 3 failures, the auditor takes over. The lead either patches the environment or escalates to the user. Agents NEVER loop indefinitely.**
...
- **Hash unchanged** → evaluator receives a pre-compiled rubric summary instead of re-deriving from raw fixtures
- **Hash changed** → evaluator re-reads and re-derives (Auditor patched something)
...
interface Store {
  ...
  snapshots: {
    ...
    // Fixture hashes (evaluator caching)
...
interface Store {
  ...
  tasks: Array<{
    ...
    feedback?: string; // evaluator/security feedback for fix tasks
    priority: number;
  }>;
...
- Auditor reads the dataset between cycles. When `recurrence >= 2`: the auditor MUST update the relevant lint rule, spec, or hygiene doc to prevent reoccurrence. Fixing the bug is not enough — fix the system.
- The `prevention` field records what systemic change was made. If empty, the auditor has not yet addressed it.
...
After every builder or fix agent completes, the evaluator runs these five checks in order.
...
Builders get: PRD, STORIES.md, FLOW_SPEC.md, COPY.md, INPUTS.md (if exists), DATA-CONTRACTS.md, and the latest HYGIENE.md. Builders do NOT get: golden fixtures, evaluator rubric, step expectations, or any file under `docs/00-canonical/fixtures/`.

| Role       | Context includes                                                     | Context excludes                                     |
| ---------- | -------------------------------------------------------------------- | ---------------------------------------------------- |
| Builder    | PRD, STORIES, FLOW_SPEC, COPY, HYGIENE, foundation spec, store state | Golden fixtures, evaluator rubric, step expectations |
| Evaluator  | Golden fixtures, step expectations, evaluator rubric, builder output | Other builders' output, Auditor analysis                |
| Security   | Full codebase (read-only), security checklist                        | Evaluator results, golden fixtures                   |
| Compliance | Builder diff, acceptance criteria, hygiene rules                     | Golden fixtures, evaluator rubric                    |
...
Golden fixtures in `docs/00-canonical/fixtures/` are **invisible to builders during development**. The evaluator loads them at eval time. This prevents builders from optimizing for tests instead of specs. Builders should implement features based on the spec, not reverse-engineer expected outputs from fixture files.
...
The Orchestrator MUST enforce this by excluding `docs/00-canonical/fixtures/` from the file list and context payload when constructing builder prompts.
...
See `docs/00-canonical/GLOSSARY.md` → Steps section for the actual component filenames (legacy numbering does not match logical step numbers).
...
- The `reviewResult` field is populated by the evaluator after review.
...
4. **Evaluation criteria** — what the evaluator and security agents check. The golden fixtures, the five-check protocol, the scoring tiers. This document.
...
- The foundation spec
- The feature spec (for builders) or evaluation criteria (for evaluator/security)
- The hygiene rules
...
- **Merge strategy** — parallel agent branches are merged sequentially. Merge conflicts are escalated to auditor.
...
If the auditor is making complex decisions on every task, it's doing the agents' work.
...
7. **Three attempts then escalate.** Never loop indefinitely. Three fix attempts, then the auditor takes over.
...
6. **Post-merge build check.** After merging all features in a phase: `npm run build`. If it fails, the auditor analyzes which merge introduced the break.
...
Branches are ephemeral. They are deleted after successful merge. Failed branches are preserved for forensics until the auditor reviews them.
