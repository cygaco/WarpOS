...
| F5 | **Teammate-ε cannot call the Agent tool (ED-041)** | The in-process roster shape is α-only; a teammate-spawned ε must route CLI-only and surface `requires-orchestrator` rows. Skill docs carried the false premise for a wave. | ED-041; contract `alpha_only_shapes` | ENFORCED (contract) / OPEN (E2 wiring) |
