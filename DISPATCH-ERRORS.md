...
| F5 | ~~**Teammate-ε cannot call the Agent tool (ED-041)**~~ — **FALSIFIED 2026-06-17 (verify-don't-inherit)** | **STALE PREMISE — does NOT reproduce on the current harness.** Live probe 2026-06-17 (operator-directed): teammate-ε spawned a general-purpose subagent, **director-of-product**, **product-lead**, AND a **depth-3 nest (ε→A→B)** — all returned cleanly, no "Agent is not available inside subagents". The ONLY block was the WarpOS `scope-contract-guard` (a deliberate policy gate) on the `visual-review` spawn — orthogonal to ED-041. So the in-process roster is **reachable from a teammate**; the α-only premise + `alpha_only_shapes` + the runtime's `requires-orchestrator` returns + the R1 "flatten the hierarchy" redesign were all built on a now-false constraint. **The one REAL constraint:** roster/build-chain spawns via the Agent tool must carry a `scopeContract` (or explicit allowed/forbiddenFiles) or the guard refuses them — so if teammate-ε conducts the roster directly, the dispatch path must supply that contract. Acting on this = epsilon.md §Dispatch Method + runtime `requires-orchestrator` change + an ADR (α/operator decision). | ED-041 **re-opened**; probe results 2026-06-17 (DOP-OK · PL-OK · LEAF-OK depth-3); scope-contract-guard is the real gate | **FALSIFIED 2026-06-17** — re-open ED-041; collapses the R1/R7 hierarchy workstream |
