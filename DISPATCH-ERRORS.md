...
| E2 | **mode_profiles never enforced (W2.13)** | `validateDispatch` called WITHOUT the mode field → `alpha_only_shapes` dead at every call site (ED-041's mechanical half never engaged). **[RESOLVED ADR-0014 W5]** — `alpha_only_shapes` is now `[]` (no shape is α-only; ε summons the roster directly), so the dead-gate concern is moot; the no-cascade invariant replaces the α-only framing. | elc-gpt2p #13 | **[RESOLVED ADR-0014 W5]** (alpha_only_shapes→[]; was OPEN/SP-002 scope) |
...
| F5 | **ED-041 ("Agent not available inside subagents") is a MISSTATEMENT — the Agent tool is a PER-SPEC capability** — **RE-VERIFIED 2026-06-18 (3 agreeing probes)** | **The Agent tool IS available to a subagent whose agent-definition explicitly lists `Agent` in `tools` (epsilon/gamma/delta/alpha/beta).** Epsilon-2 (a teammate-ε) AND a fresh plain (unnamed) `epsilon` subagent both reported `Agent` in their toolset and called `Agent(subagent_type:"cabinet")` to a real return (`ED041-OK`) — reproduced 3× (Epsilon-2 a7a1f5ca/a0a4d305 + plain-ε a8a513aa). A `general-purpose`/`*` subagent does NOT get `Agent` (the `*` catch-all excludes it — an anti-recursion measure). **The 2026-06-17 "HOLDS" re-probe was a CATEGORY ERROR: it tested `general-purpose`, not ε** — so it found `Agent` absent and wrongly generalized "subagents can't". The accurate rule: a subagent has `Agent` iff its spec grants it. So a **teammate-ε CAN call the Agent tool → CAN summon the in-process roster** (managers/leads/directors + design-quality/visual-review). The ONLY real gate is the `scope-contract-guard` (a roster/build-chain Agent-tool spawn must carry a `scopeContract`). The DUMP's "ε summons the roster directly" premise is **VALID**. **IMPLEMENTATION — RESOLVED — ADR-0014 (E-DISPATCH-PERFECT-001 W5, 2026-06-19): `alpha_only_shapes`→`[]`, epsilon.md + dispatch-contract + agent-dispatch-guide swept, no-cascade enforcer shipped** (was: relax the α-only doctrine — epsilon.md §Dispatch Method + runtime `requires-orchestrator` returns + dispatch-contract `alpha_only_shapes` + an ADR). | ED-041 **FALSIFIED for explicit-`Agent` specs** (ε/γ/δ/α/β); holds only for `*`/general-purpose subagents; re-probes 2026-06-18 (Epsilon-2 ×2 + plain-ε ×1, all case-a) | **RESOLVED — ADR-0014 (W5, 2026-06-19): alpha_only_shapes→[], epsilon.md + dispatch-contract + guide swept, no-cascade enforcer shipped.** Per-spec capability — ε (top-level OR teammate) summons the roster; the spawn-hand stays with the conductor. |
...
**Today:** α+ε+β persistent; directors/leads/reviewers are ephemeral Agent-tool rows. **[RESOLVED ADR-0014 W5 — the "ONLY α can spawn (ED-041)" premise below is SUPERSEDED: ε (top-level OR teammate) now summons the roster directly; the spawn-hand-stays-with-the-conductor invariant replaces the α-only framing, so the α-bottleneck this proposal targets is largely dissolved.]** ~~that ONLY α can spawn (ED-041)~~ — every consult funnels through the α bottleneck, each spawn re-pays full context load, and consult latency is a fresh cold start.
**Proposal:** persistent core = **α + ε + β + director-of-engineering + director-of-product (+ quality-lead through gauntlet phases)**, spawned at mode entry with plain names, message-addressable by ε DIRECTLY (teammate→teammate DM needs no Agent tool — this dissolves most of ED-041's practical pain without touching the harness limitation). **[RESOLVED ADR-0014 W5 — there is no "harness limitation": ε's spec grants `Agent`, so teammate-ε can summon the roster directly; ED-041 was a per-spec misstatement. R1's flatten-the-team value (less context cold-start) still stands on its own merits.]**
...
**Costs:** ~~spawnable only by the top-level orchestrator (ED-041)~~ **[RESOLVED ADR-0014 W5 — the ε conductor (top-level OR teammate) is itself the spawn-hand now; ED-041's α-only premise is retired. The spawn-hand-stays-with-the-conductor / no-cascade invariant is the replacement constraint]**; records depend on the new mode landing.
...
With R1 in place, revisit how consults flow: ε DMs directors directly (no α relay), β consults batch at phase boundaries (front-loaded per ED-030 lesson), and `requires-orchestrator` shrinks to genuinely-Agent-tool-only rows (design-quality/visual-review Playwright agents). α's residual roles: spawn-hand, control-point owner for dispatch-shape, operator interface, substrate. This is the operator's "less hierarchy" applied to message topology, not just team membership. **[Updated ADR-0014 W5 — the ε conductor (top-level OR teammate) is now itself the roster spawn-hand via the Agent tool; `requires-orchestrator` is the node-script→ε-agent hand-off (NOT a hand to α). α's residual roles narrow to control-point owner / operator interface / substrate; the spawn-hand stays with the conductor under the no-cascade invariant.]**
