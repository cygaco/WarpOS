...
| F5 | **ED-041 ("Agent not available inside subagents") is a MISSTATEMENT — the Agent tool is a PER-SPEC capability** — **RE-VERIFIED 2026-06-18 (3 agreeing probes)** | **The Agent tool IS available to a subagent whose agent-definition explicitly lists `Agent` in `tools` (epsilon/gamma/delta/alpha/beta).** Epsilon-2 (a teammate-ε) AND a fresh plain (unnamed) `epsilon` subagent both reported `Agent` in their toolset and called `Agent(subagent_type:"cabinet")` to a real return (`ED041-OK`) — reproduced 3× (Epsilon-2 a7a1f5ca/a0a4d305 + plain-ε a8a513aa). A `general-purpose`/`*` subagent does NOT get `Agent` (the `*` catch-all excludes it — an anti-recursion measure). **The 2026-06-17 "HOLDS" re-probe was a CATEGORY ERROR: it tested `general-purpose`, not ε** — so it found `Agent` absent and wrongly generalized "subagents can't". The accurate rule: a subagent has `Agent` iff its spec grants it. So a **teammate-ε CAN call the Agent tool → CAN summon the in-process roster** (managers/leads/directors + design-quality/visual-review). The ONLY real gate is the `scope-contract-guard` (a roster/build-chain Agent-tool spawn must carry a `scopeContract`). The DUMP's "ε summons the roster directly" premise is **VALID**. **IMPLEMENTATION (relax the α-only doctrine — epsilon.md §Dispatch Method + runtime `requires-orchestrator` returns + dispatch-contract `alpha_only_shapes` + an ADR) = NEXT-SESSION task** (substantial design change; do carefully + supply the scopeContract). | ED-041 **FALSIFIED for explicit-`Agent` specs** (ε/γ/δ/α/β); holds only for `*`/general-purpose subagents; re-probes 2026-06-18 (Epsilon-2 ×2 + plain-ε ×1, all case-a) | **per-spec capability** — ε CAN summon the roster; the α-only doctrine-relaxation is the next-session implementation |
